"""Test script for the OLD OutletDeleteBC with backflow_clamp.

Runs a short HP2D simulation and reports per-step diagnostics to
compare outlet behaviour against the new OutletBufferedDeleteBC.

Usage:
    cd cases_dynamic/Hagen_Poiseuile
    python test_outlet_old_bc.py
"""
...
# --- BCs: OLD approach (OutletDeleteBC + backflow_clamp) ---
...
bc_set.add(
    OutletDeleteBC(
        outlet_pos=L + outlet_buffer, axis=0, bV=bV,
        backflow_clamp=2.0,
    ),
    None,
)
...
print(f"{'Step':>6}  {'t':>7}  {'Verts':>5}  {'bV':>3}  "
      f"{'min_ux':>10}  {'max_ux':>10}  {'backward':>8}  {'on_bdy_outlet':>13}")
print("-" * 85)
...
def callback(step, t, HC, bV, diag):
    ...
    on_bdy = sum(1 for v in topo_bdy
                 if v.x_a[0] > L - 1.0 and not wall_criterion(v))
    ...
    print(f"{step:6d}  {t:7.3f}  {n:5d}  {len(bV):3d}  "
          f"{min_ux:10.4f}  {max_ux:10.4f}  {backward:8d}  {on_bdy:13d}")
...
backward_final = sum(1 for v in HC.V
                     if v.u[0] < -0.01 and not wall_criterion(v))
print(f"\nFinal: {n_final} verts, t={t_final:.4f}, backward={backward_final}")
