"""
Incompressible Poiseuille Flow — Plug Flow → Parabolic Development with pressure Poisson projection
"""
...
def plot_frame(step, t, HC, fig_dir,
    ...
    ax_err.set_title(f'Residuals: L2_SS={l2_ss:.2e}  div_L2={ts["div_u_l2"][-1]:.1e}' if ts.get('div_u_l2') else f'Residuals: L2_SS={l2_ss:.2e}', fontsize=11)
...
ts = dict(
    ...
    div_u_l2=[], div_u_linf=[],   # incompressibility residuals (AFTER projection)
    div_u_pre_l2=[],              # L2 div u BEFORE projection
    phi_max=[],                   # max pressure correction |phi| [Pa]
)
...
while t < T_END - 1e-14:
    ...
    #     Computes d^2 phi/dy^2 = (rho/dt)*div u*, corrects u_y
    #     and adds phi to pressure. Keeps u_x unchanged (x-homogeneous).
