#!/usr/bin/env python3
"""3D dam break — liquid only with an implicit atmospheric free surface.

The mesh covers ONLY the water column (``col_w x col_h x col_d``).
Bottom, upstream (x=0), and both z walls are frozen no-slip walls.
The top face (y=col_h) and the downstream face (x=col_w) are free
surfaces whose vertices advect freely under gravity.

No explicit air phase: absolute pressure is tracked by the linear
Tait–Murnaghan EOS, initialised from the hydrostatic profile
``P(y) = P_atm + rho_l * g * (col_h - y)``.

Produces:
  fig/dam_break_3D_no_air_fluid.png
  fig/dam_break_3D_no_air.mp4
  results/snapshots_3D_no_air/
...
_SNAPSHOTS = os.path.join(_RESULTS, 'snapshots_3D_no_air')
...
def main():
    ...
    print(f"Column: {col_w:.3f} x {col_h:.3f} x {col_d:.3f} m")
    ...
    # 3D free-surface corners produce large spurious forces in the
    # current DDG FVM (see FEATURES.md: "multiphase dual volume
    # splitting").  We catch integration errors here so that the
    ...
    try:
        ...
        if len(t_hist) > 1:
            ...
            ax.set_xlabel('t [s]')
            ax.set_ylabel('KE [J]')
            ax.set_title('3D Dam Break (no air) — kinetic energy')
            ax.grid(True, alpha=0.3)
            fig.tight_layout()
            fig.savefig(
                os.path.join(_FIG, 'dam_break_3D_no_air_energy.png'),
                dpi=150,
            )
            plt.close(fig)
    ...
    if history.n_snapshots > 1:
        try:
            dynamic_plot_fluid(
                history, HC, bV=bV,
                save_path=os.path.join(_FIG, 'dam_break_3D_no_air.mp4'),
                fps=15, dpi=100,
            )
            print(f"Animation saved to fig/dam_break_3D_no_air.mp4")
        except Exception as e:
