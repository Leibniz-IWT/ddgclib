#!/usr/bin/env python3
"""3D electrolysis hydrogen-bubble dynamic simulation.

Three-dimensional version of ``electrolysis_bubble_2D.py``: a gas
bubble grows on a flat electrode (z = -L_domain) via a linear mass
generation rate (placeholder for electrochemistry) and detaches under
buoyancy once its Bond number exceeds the Fritz limit.

Produces:
  fig/electrolysis_bubble_3D.mp4       -- animation
  fig/electrolysis_bubble_3D_R.png     -- R_eq(t) vs Fritz detachment
  fig/electrolysis_bubble_3D_z.png     -- bubble centroid height
...
from cases_dynamic.electrolysis_bubble.src._params import (
    ...
    n_refine_outer_3d, n_refine_drop_3d, t_end_3d, dm_dt_3d,
    cfl_safety,
)
...
def main():
    ...
    dt_cfl = 0.5 * cfl_safety * dx_min / c_s  # 2x tighter for 3D
    dt_st = 0.2 * np.sqrt(rho_liq * dx_min**3 / gamma)
    dt = min(dt_cfl, dt_st)
    n_steps = int(t_end_3d / dt) + 1
    record_every = max(1, n_steps // 120)
    print(f"c_s = max({c_s_liq:.2f}, {c_s_gas:.2f}) = {c_s:.2f} m/s")
    print(f"dx_min = {dx_min:.3e}, dt_cfl = {dt_cfl:.2e}, "
          f"dt_st = {dt_st:.2e}")
    print(f"dt = {dt:.2e}, n_steps = {n_steps}, t_end = {t_end_3d}")
    ...
    print(f"Initial gas volume: {diag_list[-1]['gas_volume']:.3e} m^3, "
    ...
    def callback(step, t, HC_cb, bV_cb=None, diagnostics=None):
        ...
        inject_gas_mass(HC_cb, mps, dm_dt=dm_dt_3d, dt=dt, gas_phase=1)
        if step % record_every == 0:
            ...
            if dz > 0.3 * R0 and detached_step[0] is None:
    ...
    try:
        ...
        plot_radius_history(t_arr, R_arr, R0, R_detach=R_fritz, ax=ax,
                            title="3D H2 bubble radius (electrolysis)")
        fig.savefig(os.path.join(_FIG, 'electrolysis_bubble_3D_R.png'),
                    dpi=150)
        plt.close(fig)
        ...
        plot_centroid_history(t_arr, z_arr, electrode_level,
                              ...
                              title="3D bubble centroid rise")
        fig.savefig(os.path.join(_FIG, 'electrolysis_bubble_3D_z.png'),
                    dpi=150)
        plt.close(fig)
    ...
    if history.n_snapshots > 1:
        try:
            dynamic_plot_fluid(
                history, HC, bV=bV,
                save_path=os.path.join(_FIG, 'electrolysis_bubble_3D.mp4'),
                fps=15, dpi=100,
                phase_field='phase',
                interface_field='is_interface',
            )
            print(f"Animation saved to fig/electrolysis_bubble_3D.mp4")
