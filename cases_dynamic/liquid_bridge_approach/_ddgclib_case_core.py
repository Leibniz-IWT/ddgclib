...
# PR33 operator dependency used by cases 9-12:
...
USER_SOLVER_PR33_FREE_SURFACE_DIRICHLET_PRESSURE = False
...
USER_REPORTED_GORGE_ADD_WALL_VISCOUS_FORCE = False
USER_REPORTED_GORGE_ADD_COX_CL_FORCE = False
USER_REPORTED_GORGE_ADD_PR33_WALL_PRESSURE_FORCE = False
USER_REPORTED_GORGE_ADD_LUBRICATION_PRESSURE_FORCE = False
USER_REPORTED_GORGE_LUBRICATION_PRESSURE_WALL_TRACTION = False
USER_SOLVER_ADD_LUBRICATION_PRESSURE_FORCE = False
USER_LUBRICATION_PRESSURE_SCALE = 1.0
USER_LUBRICATION_PRESSURE_SCALE_POINTS = ()
USER_REPORTED_APPROACH_SOLID_LINE_FORCE = False
USER_REPORTED_APPROACH_SOLID_LINE_START_D_OVER_R = 0.04
USER_REPORTED_APPROACH_SOLID_LINE_FULL_D_OVER_R = 0.025
...
class VolumetricPitoisConfig:
    ...
    sparse_history_only: bool = False
    ...
    solver_pr33_free_surface_dirichlet_pressure: bool = USER_SOLVER_PR33_FREE_SURFACE_DIRICHLET_PRESSURE
    ...
    gorge_neck_fit_radius_max_raw_ratio: float = 5.0
    ...
    reported_gorge_add_wall_viscous_force: bool = USER_REPORTED_GORGE_ADD_WALL_VISCOUS_FORCE
    reported_gorge_add_cox_cl_force: bool = USER_REPORTED_GORGE_ADD_COX_CL_FORCE
    reported_gorge_add_pr33_wall_pressure_force: bool = USER_REPORTED_GORGE_ADD_PR33_WALL_PRESSURE_FORCE
    reported_gorge_add_lubrication_pressure_force: bool = USER_REPORTED_GORGE_ADD_LUBRICATION_PRESSURE_FORCE
    reported_gorge_lubrication_pressure_wall_traction: bool = (
        USER_REPORTED_GORGE_LUBRICATION_PRESSURE_WALL_TRACTION
    )
    solver_add_lubrication_pressure_force: bool = USER_SOLVER_ADD_LUBRICATION_PRESSURE_FORCE
    lubrication_pressure_scale: float = USER_LUBRICATION_PRESSURE_SCALE
    lubrication_pressure_scale_points: tuple = USER_LUBRICATION_PRESSURE_SCALE_POINTS
    reported_approach_solid_line_force: bool = USER_REPORTED_APPROACH_SOLID_LINE_FORCE
    reported_approach_solid_line_start_d_over_r: float = USER_REPORTED_APPROACH_SOLID_LINE_START_D_OVER_R
    reported_approach_solid_line_full_d_over_r: float = USER_REPORTED_APPROACH_SOLID_LINE_FULL_D_OVER_R
    ...
    def __post_init__(self) -> None:
        ...
        if gorge_pressure_model not in {
            ...
            raise ValueError(
                "gorge_pressure_model must be axisym_fit, axisym_initial, "
                "axisym_initial_pr33_neck, fheron_neck, axisym_fheron_floor, "
                "or axisym_initial_fheron_neck."
            )
        ...
        if (
            not np.isfinite(float(self.gorge_neck_fit_radius_max_raw_ratio))
            or float(self.gorge_neck_fit_radius_max_raw_ratio) <= 0.0
        ):
            raise ValueError("gorge_neck_fit_radius_max_raw_ratio must be positive and finite.")
        if (
            not np.isfinite(float(self.lubrication_pressure_scale))
            or float(self.lubrication_pressure_scale) < 0.0
        ):
            raise ValueError("lubrication_pressure_scale must be finite and >= 0.")
        scale_points = tuple(getattr(self, "lubrication_pressure_scale_points", ()) or ())
        for item in scale_points:
            if len(item) != 2:
                raise ValueError("lubrication_pressure_scale_points entries must be (D/R, scale).")
            d_value = float(item[0])
            scale_value = float(item[1])
            if (not np.isfinite(d_value)) or d_value < 0.0:
                raise ValueError("lubrication_pressure_scale_points D/R values must be finite and >= 0.")
            if (not np.isfinite(scale_value)) or scale_value < 0.0:
                raise ValueError("lubrication_pressure_scale_points scale values must be finite and >= 0.")
        object.__setattr__(
            self,
            "lubrication_pressure_scale_points",
            tuple((float(d), float(s)) for d, s in scale_points),
        )
        if not np.isfinite(float(self.reported_approach_solid_line_start_d_over_r)):
            raise ValueError("reported_approach_solid_line_start_d_over_r must be finite.")
        if not np.isfinite(float(self.reported_approach_solid_line_full_d_over_r)):
            raise ValueError("reported_approach_solid_line_full_d_over_r must be finite.")
...
def _update_pressure_scalar(state: VolumetricPitoisState) -> None:
    ...
    base_pressure_model = "axisym_initial" if pressure_model == "axisym_initial_pr33_neck" else pressure_model
    use_fheron_after_initial = (
        base_pressure_model == "axisym_initial_fheron_neck"
        and float(getattr(state, "elapsed_time_s", 0.0)) > 0.0
    )
    if base_pressure_model == "fheron_neck" or use_fheron_after_initial:
    ...
    if kappa_fit is not None:
        ...
        if base_pressure_model == "axisym_initial":
            if not hasattr(state, "initial_axisym_pressure_scalar"):
                state.initial_axisym_pressure_scalar = float(axisym_pressure)
            state.last_pressure_source = "axisym_initial"
            ...
            state.pressure_scalar = float(state.initial_axisym_pressure_scalar)
            return
        if base_pressure_model == "axisym_fheron_floor":
            fheron_pressure = _fheron_neck_pressure_scalar(state)
...
def _reported_wall_pressure_model(v, *, HC=None, dim: int = 3, state: VolumetricPitoisState):
    """Pressure used by the raw wall Cauchy-traction force report."""
    pressure = float(_pressure_model(v, HC=HC, dim=dim, state=state))
    if not bool(
        getattr(
            state.config,
            "reported_gorge_add_lubrication_pressure_force",
            USER_REPORTED_GORGE_ADD_LUBRICATION_PRESSURE_FORCE,
        )
    ):
        return pressure
    profile = _lubrication_pressure_profile(state)
    if profile is None:
        return pressure
    p_lub_pa = _lubrication_pressure_at_point_pa(
        state,
        np.asarray(v.x_a[:3], dtype=float),
        profile=profile,
    )
    return pressure + float(_scale_to_compute(state.config, p_lub_pa, "pressure_scale_pa"))
...
def _Ftot(
    ...
    *,
    ...
    if bool(
        getattr(
            state.config,
            "solver_add_lubrication_pressure_force",
            USER_SOLVER_ADD_LUBRICATION_PRESSURE_FORCE,
        )
    ):
        Ftot += _solver_lubrication_pressure_force(v, state=state)
...
def _axisym_clear_caches(state: VolumetricPitoisState) -> None:
    for attr in (
        ...
        "_lubrication_pressure_profile_cache",
        "_gmsh_lubrication_pressure_force_cache",
    ):
...
def _axisym_force_cache_key(
    ...
    *,
    ...
    return (
        ...
        round(float(getattr(state.config, "lubrication_pressure_scale", USER_LUBRICATION_PRESSURE_SCALE)), 18),
    )
...
def _apply_gmsh_incompressible_projection(state: VolumetricPitoisState, *, dt: float) -> None:
    ...
    if closure == "compressible" and not bool(
        ...
        if bool(getattr(state.config, "solver_pr33_free_surface_dirichlet_pressure", False)):
            free_surface_cells = _pr33_free_surface_cell_ids(state, n_cells=int(tets.shape[0]))
            state.last_solver_pr33_free_surface_dirichlet_cells = int(free_surface_cells.size)
            if free_surface_cells.size:
                matrix_lil = matrix.tolil()
                for cell_id in free_surface_cells:
                    matrix_lil.rows[int(cell_id)] = [int(cell_id)]
                    matrix_lil.data[int(cell_id)] = [1.0]
                    rhs[int(cell_id)] = 0.0
                matrix = matrix_lil.tocsr()
        else:
            state.last_solver_pr33_free_surface_dirichlet_cells = 0
        pressure = np.asarray(spsolve(matrix, rhs), dtype=float)
...
def _gorge_neck_radius_and_center(state: VolumetricPitoisState) -> tuple[float, np.ndarray]:
    ...
    if (
        ...
        and 0.20 * waist_radius_raw
        <= waist_radius_fit
        <= float(getattr(state.config, "gorge_neck_fit_radius_max_raw_ratio", 5.0)) * waist_radius_raw
    ):
...
def _solid_contact_line_surface_tension_forces(state: VolumetricPitoisState) -> tuple[np.ndarray, np.ndarray]:
    """Axial surface-tension reaction at the wetted solid contact line."""
    _bottom_center, _top_center, axis = _particle_centers_physical(state)
    radius = max(float(state.config.particle_radius), 1.0e-30)

    def one(which: str) -> np.ndarray:
        ring = state.top_contact_ring if which == "top" else state.bottom_contact_ring
        contact_radius = float(_cap_radius(ring)) if ring else 0.0
        if contact_radius <= 0.0 or not np.isfinite(contact_radius):
            return np.zeros(3, dtype=float)
        beta = float(np.arcsin(np.clip(contact_radius / radius, 0.0, 1.0)))
        magnitude = 2.0 * np.pi * float(state.config.gamma) * contact_radius * float(np.sin(beta))
        sign = -1.0 if which == "top" else 1.0
        return sign * magnitude * axis
...
def _approach_solid_line_blend_weight(state: VolumetricPitoisState) -> float:
    start = float(
        getattr(
            state.config,
            "reported_approach_solid_line_start_d_over_r",
            USER_REPORTED_APPROACH_SOLID_LINE_START_D_OVER_R,
        )
    )
    full = float(
        getattr(
            state.config,
            "reported_approach_solid_line_full_d_over_r",
            USER_REPORTED_APPROACH_SOLID_LINE_FULL_D_OVER_R,
        )
    )
    if start <= full:
        return 1.0 if (_gap(state) / max(float(state.config.particle_radius), 1.0e-30)) <= full else 0.0
    d_over_r = _gap(state) / max(float(state.config.particle_radius), 1.0e-30)
    return float(np.clip((start - d_over_r) / max(start - full, 1.0e-30), 0.0, 1.0))


def _pr33_neck_pressure_delta(state: VolumetricPitoisState) -> float:
    """Median raw PR33 pressure on tetrahedra adjacent to the gorge ring."""
    pressure = np.asarray(
        getattr(
            state,
            "ddgclib_closure_pressure_delta",
            getattr(state, "incompressible_projection_pressure", np.empty(0, dtype=float)),
        ),
        dtype=float,
    ).reshape(-1)
    tets = np.asarray(getattr(state, "volume_export_tets", np.empty((0, 4), dtype=int)), dtype=int).reshape((-1, 4))
    ...
    if pressure.size == 0 or tets.size == 0 or not vertices or not waist_ring:
        state.last_gorge_report_pr33_neck_pressure_delta = 0.0
        state.last_gorge_report_pr33_bridge_pressure_reference = 0.0
        state.last_gorge_report_pr33_neck_pressure_samples = 0
        return 0.0

    vertex_index = {id(vertex): idx for idx, vertex in enumerate(vertices)}
    waist_indices = [
        int(vertex_index[id(vertex)])
        for vertex in waist_ring
        if id(vertex) in vertex_index
    ]
    ...
    n_pressure_tets = min(int(tets.shape[0]), int(pressure.size))
    tets = tets[:n_pressure_tets]
    pressure = pressure[:n_pressure_tets]
    valid_tets = (np.min(tets, axis=1) >= 0) & (np.max(tets, axis=1) < len(vertices))
    if not np.any(valid_tets):
        state.last_gorge_report_pr33_neck_pressure_delta = 0.0
        state.last_gorge_report_pr33_bridge_pressure_reference = 0.0
        state.last_gorge_report_pr33_neck_pressure_samples = 0
    ...
    waist_mask = np.isin(tets, np.asarray(waist_indices, dtype=int)).any(axis=1)
    neck_samples = pressure[valid_tets & waist_mask]
    neck_samples = neck_samples[np.isfinite(neck_samples)]
    bridge_samples = pressure[valid_tets]
    bridge_samples = bridge_samples[np.isfinite(bridge_samples)]
    if neck_samples.size == 0 or bridge_samples.size == 0:
        state.last_gorge_report_pr33_neck_pressure_delta = 0.0
        state.last_gorge_report_pr33_bridge_pressure_reference = 0.0
        state.last_gorge_report_pr33_neck_pressure_samples = 0
        return 0.0

    bridge_reference = float(np.median(bridge_samples))
    delta = float(np.median(neck_samples) - bridge_reference)
    state.last_gorge_report_pr33_neck_pressure_delta = delta
    state.last_gorge_report_pr33_bridge_pressure_reference = bridge_reference
    state.last_gorge_report_pr33_neck_pressure_samples = int(neck_samples.size)
    return delta


def _gorge_report_pressure_scalar(state: VolumetricPitoisState) -> float:
    pressure = float(state.pressure_scalar)
    if (
        str(getattr(state.config, "gorge_pressure_model", USER_GORGE_PRESSURE_MODEL)).strip().lower()
        == "axisym_initial_pr33_neck"
    ):
        pressure += _pr33_neck_pressure_delta(state)
    else:
        state.last_gorge_report_pr33_neck_pressure_delta = 0.0
        state.last_gorge_report_pr33_bridge_pressure_reference = 0.0
        state.last_gorge_report_pr33_neck_pressure_samples = 0
    state.last_gorge_report_pressure_scalar = float(pressure)
    return float(pressure)


def _solver_pr33_sphere_cap_force(state: VolumetricPitoisState, *, which: str) -> np.ndarray:
    force_map = getattr(state, "solver_pr33_pressure_force_map", {})
    if not force_map:
        return np.zeros(3, dtype=float)
    if which == "top":
        vertices = list(getattr(state, "cap_top_interior", [])) + list(getattr(state, "top_contact_ring", []))
    else:
        vertices = list(getattr(state, "cap_bottom_interior", [])) + list(getattr(state, "bottom_contact_ring", []))
    ...
    for vertex in vertices:
        ...
        if vid in seen:
            continue
        seen.add(vid)
        value = force_map.get(vid)
        if value is not None:
            force += np.asarray(value[:3], dtype=float)
    return np.nan_to_num(force, nan=0.0, posinf=0.0, neginf=0.0)


def _cox_contact_line_solid_reaction_force(state: VolumetricPitoisState, *, which: str) -> np.ndarray:
    if not bool(getattr(state.config, "use_cox_contact_line_force", USER_USE_COX_CONTACT_LINE_FORCE)):
        return np.zeros(3, dtype=float)
    force_map = _cox_uy_contact_line_liquid_force_map(state)
    ring = state.top_contact_ring if which == "top" else state.bottom_contact_ring
    force_on_liquid = np.zeros(3, dtype=float)
    for vertex in ring:
        value = force_map.get(id(vertex))
        if value is not None:
            force_on_liquid += np.asarray(value[:3], dtype=float)
    return np.nan_to_num(-force_on_liquid, nan=0.0, posinf=0.0, neginf=0.0)


def _pr33_pressure_array_for_report(state: VolumetricPitoisState) -> np.ndarray:
    pressure = np.asarray(
        getattr(
            state,
            "ddgclib_closure_pressure",
            getattr(state, "incompressible_projection_pressure", np.empty(0, dtype=float)),
        ),
        dtype=float,
    ).reshape(-1)
...
def _pr33_free_surface_pressure_reference(
    state: VolumetricPitoisState,
    *,
    pressure: np.ndarray,
    ...
    side_tris = np.asarray(getattr(state, "surface_export_side_tris", np.empty((0, 3), dtype=int)), dtype=int)
    if pressure.size == 0 or side_tris.size == 0:
        state.last_pr33_free_surface_pressure_reference = 0.0
        state.last_pr33_free_surface_pressure_reference_samples = 0
        return 0.0
    ...
    for tri in side_tris.reshape((-1, 3)):
        ...
        cell_id = face_owner.get(key)
        if cell_id is None or int(cell_id) < 0 or int(cell_id) >= pressure.size:
            continue
        value = float(pressure[int(cell_id)])
    ...
    if not samples:
        ...
        state.last_pr33_free_surface_pressure_reference_samples = 0
        return 0.0
    reference = float(np.median(np.asarray(samples, dtype=float)))
    state.last_pr33_free_surface_pressure_reference = reference
    state.last_pr33_free_surface_pressure_reference_samples = int(len(samples))
    return reference


def _pr33_free_surface_cell_ids(
    state: VolumetricPitoisState,
    *,
    n_cells: int,
    face_owner: dict[tuple[int, int, int], int] | None = None,
    ...
    side_tris = np.asarray(getattr(state, "surface_export_side_tris", np.empty((0, 3), dtype=int)), dtype=int)
    if int(n_cells) <= 0 or side_tris.size == 0:
    ...
    for tri in side_tris.reshape((-1, 3)):
        key = tuple(sorted(int(v) for v in tri[:3]))
        cell_id = face_owner.get(key)
        if cell_id is not None and 0 <= int(cell_id) < int(n_cells):
            cells.add(int(cell_id))
    return np.asarray(sorted(cells), dtype=int)


def _gmsh_pr33_wall_pressure_cap_force(
    state: VolumetricPitoisState,
    *,
    which: str,
    face_owner: dict[tuple[int, int, int], int] | None = None,
) -> np.ndarray:
    pressure = _pr33_pressure_array_for_report(state)
    tets = np.asarray(getattr(state, "volume_export_tets", np.empty((0, 4), dtype=int)), dtype=int).reshape((-1, 4))
    n_cells = min(int(pressure.size), int(tets.shape[0]))
    if n_cells <= 0:
        return np.zeros(3, dtype=float)
    pressure = pressure[:n_cells]
    tets = tets[:n_cells]
    if face_owner is None:
        face_owner = _gmsh_tet_face_owner_map(state)
    reference = _pr33_free_surface_pressure_reference(state, pressure=pressure, face_owner=face_owner)
    sigma = -(pressure - reference)[:, None, None] * np.eye(3, dtype=float)[None, :, :]
    ...
    return _gmsh_wall_stress_cap_traction_force(
        state,
        which=which,
        data=data,
        face_owner=face_owner,
        diag_key="pr33_wall_pressure",
    )
...
def _gorge_pressure_cap_forces(state: VolumetricPitoisState) -> tuple[np.ndarray, np.ndarray]:
    ...
    if waist_radius <= 0.0 or not np.isfinite(waist_radius):
        return np.zeros(3, dtype=float), np.zeros(3, dtype=float)
    pressure = _gorge_report_pressure_scalar(state)
...
def _gmsh_wall_stress_cap_traction_force(
    ...
    *,
    ...
    def set_diag(*, total_faces: int, matched_faces: int, total_area: float, matched_area: float) -> None:
        setattr(state, f"last_gmsh_{diag_key}_{which}_faces", int(total_faces))
        setattr(state, f"last_gmsh_{diag_key}_{which}_matched_faces", int(matched_faces))
        setattr(state, f"last_gmsh_{diag_key}_{which}_area_m2", float(total_area))
        setattr(state, f"last_gmsh_{diag_key}_{which}_matched_area_m2", float(matched_area))

    if data is None:
        pressure_model = lambda vv, HC=None, dim=3: _reported_wall_pressure_model(
            vv,
            HC=HC,
            dim=dim,
            state=state,
...
def _sphere_total_forces(state: VolumetricPitoisState) -> dict[str, np.ndarray]:
    if bool(getattr(state, "gmsh_compute_mesh", False)):
        ...
        add_wall_viscous = bool(
            getattr(
                state.config,
                "reported_gorge_add_wall_viscous_force",
                USER_REPORTED_GORGE_ADD_WALL_VISCOUS_FORCE,
            )
        )
        add_cox_cl = bool(
            getattr(
                state.config,
                "reported_gorge_add_cox_cl_force",
                USER_REPORTED_GORGE_ADD_COX_CL_FORCE,
            )
        )
        add_pr33_wall_pressure = bool(
            getattr(
                state.config,
                "reported_gorge_add_pr33_wall_pressure_force",
                USER_REPORTED_GORGE_ADD_PR33_WALL_PRESSURE_FORCE,
            )
        )
        add_lubrication_pressure = bool(
            getattr(
                state.config,
                "reported_gorge_add_lubrication_pressure_force",
                USER_REPORTED_GORGE_ADD_LUBRICATION_PRESSURE_FORCE,
        ...
        top_wall_viscous_cap = np.zeros(3, dtype=float)
        bottom_wall_viscous_cap = np.zeros(3, dtype=float)
        top_pr33_wall_pressure_cap = np.zeros(3, dtype=float)
        bottom_pr33_wall_pressure_cap = np.zeros(3, dtype=float)
        top_lubrication_pressure_cap = np.zeros(3, dtype=float)
        bottom_lubrication_pressure_cap = np.zeros(3, dtype=float)
        top_cox_cl = np.zeros(3, dtype=float)
        bottom_cox_cl = np.zeros(3, dtype=float)
        face_owner = None
        stress_data = None
        ...
        if add_wall_viscous:
            if face_owner is None:
                face_owner = _gmsh_tet_face_owner_map(state)
            viscous_data = _gmsh_wall_stress_tensor_data(
                state,
                pressure_model=None,
                include_pressure=False,
                include_viscous=True,
            )
            top_wall_viscous_cap = _gmsh_wall_stress_cap_traction_force(
                state,
                which="top",
                data=viscous_data,
                face_owner=face_owner,
                diag_key="wall_viscous",
            )
            bottom_wall_viscous_cap = _gmsh_wall_stress_cap_traction_force(
                state,
                which="bottom",
                data=viscous_data,
                face_owner=face_owner,
        ...
        if add_pr33_wall_pressure:
            if face_owner is None:
                face_owner = _gmsh_tet_face_owner_map(state)
            top_pr33_wall_pressure_cap = _gmsh_pr33_wall_pressure_cap_force(
                state,
                which="top",
                face_owner=face_owner,
            )
            bottom_pr33_wall_pressure_cap = _gmsh_pr33_wall_pressure_cap_force(
                state,
                which="bottom",
                face_owner=face_owner,
            )
        if add_lubrication_pressure:
            top_lubrication_pressure_cap, bottom_lubrication_pressure_cap = (
                _lubrication_pressure_cap_forces(state)
            )
        ...
        if add_cox_cl:
            top_cox_cl = _cox_contact_line_solid_reaction_force(state, which="top")
            bottom_cox_cl = _cox_contact_line_solid_reaction_force(state, which="bottom")
            top_line += top_cox_cl
            bottom_line += bottom_cox_cl
        if bool(getattr(state.config, "reported_capillary_line_force", USER_REPORTED_CAPILLARY_LINE_FORCE)):
            ...
            if bool(
                getattr(
                    state.config,
                    "reported_approach_solid_line_force",
                    USER_REPORTED_APPROACH_SOLID_LINE_FORCE,
                )
            ):
                top_solid_line, bottom_solid_line = _solid_contact_line_surface_tension_forces(state)
                blend = _approach_solid_line_blend_weight(state)
                top_gorge_surface_tension = (
                    (1.0 - blend) * top_gorge_surface_tension + blend * top_solid_line
                )
                bottom_gorge_surface_tension = (
                    (1.0 - blend) * bottom_gorge_surface_tension + blend * bottom_solid_line
                )
        ...
        if bool(
            ...
            top_line = top_cox_cl + top_capillary_line + top_gorge_surface_tension
            bottom_capillary_line = -_ring_surface_tension_force(
                state.bottom_contact_ring,
                gamma=state.config.gamma,
                state=state,
            )
            bottom_line = bottom_cox_cl + bottom_capillary_line + bottom_gorge_surface_tension
        ...
        if bool(
            ...
            top_wall_viscous_cap = float(np.dot(top_wall_viscous_cap, axis)) * axis
            bottom_wall_viscous_cap = float(np.dot(bottom_wall_viscous_cap, axis)) * axis
            top_pr33_wall_pressure_cap = float(np.dot(top_pr33_wall_pressure_cap, axis)) * axis
            bottom_pr33_wall_pressure_cap = float(np.dot(bottom_pr33_wall_pressure_cap, axis)) * axis
            top_lubrication_pressure_cap = float(np.dot(top_lubrication_pressure_cap, axis)) * axis
            bottom_lubrication_pressure_cap = float(np.dot(bottom_lubrication_pressure_cap, axis)) * axis
            ...
            top_cox_cl = float(np.dot(top_cox_cl, axis)) * axis
            bottom_cox_cl = float(np.dot(bottom_cox_cl, axis)) * axis
        ...
        top_cap = (
            ...
            + top_wall_viscous_cap
            + top_pr33_wall_pressure_cap
            + top_lubrication_pressure_cap
        )
        bottom_cap = (
            bottom_wall_cap
            if use_wall_stress
            else bottom_gorge_cap
            + bottom_wall_viscous_cap
            + bottom_pr33_wall_pressure_cap
            + bottom_lubrication_pressure_cap
        )
        if use_wall_stress:
            force_model = "gmsh_wall_stress"
        elif (
            (add_wall_viscous or add_pr33_wall_pressure or add_lubrication_pressure or add_cox_cl)
            and bool(getattr(state.config, "reported_capillary_line_force", False))
        ):
            parts = ["gorge_pressure"]
            if add_pr33_wall_pressure:
                parts.append("pr33_wall_pressure")
            if add_lubrication_pressure:
                parts.append("lubrication_pressure")
            if add_wall_viscous:
                parts.append("wall_viscous")
            if add_cox_cl:
                parts.append("cox_cl")
            parts.append("neck_surface_tension")
            force_model = "_plus_".join(parts)
        elif add_wall_viscous and bool(getattr(state.config, "reported_capillary_line_force", False)):
            force_model = "gorge_pressure_plus_wall_viscous_plus_neck_surface_tension"
        elif add_cox_cl and bool(getattr(state.config, "reported_capillary_line_force", False)):
            force_model = "gorge_pressure_plus_cox_cl_plus_neck_surface_tension"
        elif add_wall_viscous:
            force_model = "gorge_pressure_plus_wall_viscous"
        ...
        return {
            ...
            "top_cox_contact_line": top_cox_cl,
            "bottom_cox_contact_line": bottom_cox_cl,
            ...
            "top_wall_viscous_cap": top_wall_viscous_cap,
            "bottom_wall_viscous_cap": bottom_wall_viscous_cap,
            "top_pr33_wall_pressure_cap": top_pr33_wall_pressure_cap,
            "bottom_pr33_wall_pressure_cap": bottom_pr33_wall_pressure_cap,
            "top_lubrication_pressure_cap": top_lubrication_pressure_cap,
            "bottom_lubrication_pressure_cap": bottom_lubrication_pressure_cap,
            "top_wall_total": top_wall_cap + top_line,
            "bottom_wall_total": bottom_wall_cap + bottom_line,
            "top_wall_viscous_total": top_wall_viscous_cap + top_line,
            "bottom_wall_viscous_total": bottom_wall_viscous_cap + bottom_line,
            "top_pr33_wall_pressure_total": top_pr33_wall_pressure_cap + top_line,
            "bottom_pr33_wall_pressure_total": bottom_pr33_wall_pressure_cap + bottom_line,
            "top_lubrication_pressure_total": top_lubrication_pressure_cap + top_line,
            "bottom_lubrication_pressure_total": bottom_lubrication_pressure_cap + bottom_line,
...
def _lubrication_pressure_effective_scale(state: VolumetricPitoisState) -> float:
    """Return scalar or gap-interpolated lubrication pressure scale."""
    fallback = max(
        float(getattr(state.config, "lubrication_pressure_scale", USER_LUBRICATION_PRESSURE_SCALE)),
        0.0,
    )
    points = tuple(getattr(state.config, "lubrication_pressure_scale_points", ()) or ())
    if not points:
        return fallback
    clean = sorted(
        (
            (float(d_over_r), max(float(scale), 0.0))
            for d_over_r, scale in points
            if np.isfinite(float(d_over_r)) and np.isfinite(float(scale))
        ),
    ...
    if not clean:
        return fallback
    d_over_r = _gap(state) / max(float(state.config.particle_radius), 1.0e-30)
    d_over_r = float(np.clip(d_over_r, 0.0, float("inf")))
    if d_over_r <= clean[0][0]:
        return float(clean[0][1])
    if d_over_r >= clean[-1][0]:
        return float(clean[-1][1])
    for (d0, s0), (d1, s1) in zip(clean[:-1], clean[1:]):
        if d0 <= d_over_r <= d1:
            w = (d_over_r - d0) / max(d1 - d0, 1.0e-30)
            return float((1.0 - w) * s0 + w * s1)
    return fallback


def _lubrication_pressure_profile(state: VolumetricPitoisState) -> SimpleNamespace | None:
    """Return the finite-volume squeeze-film pressure profile in physical units."""
    if hasattr(state, "_lubrication_pressure_profile_cache"):
        return getattr(state, "_lubrication_pressure_profile_cache")

    config = state.config
    _bottom_center, _top_center, axis = _particle_centers_physical(state)
    radius_m = max(float(_length_from_compute(config, float(config.particle_radius))), 1.0e-30)
    gap_m = max(float(_length_from_compute(config, _gap(state))), 1.0e-12)
    mu_pas = max(float(_viscosity_from_compute(config, float(config.mu_f))), 0.0)
    dhdt_mps = float(_velocity_from_compute(config, float(getattr(config, "cap_speed", 0.0))))
    volume_m3 = max(float(_volume_from_compute(config, _snapshot_msh_volume_m3(state))), 0.0)
    if mu_pas <= 0.0 or abs(dhdt_mps) <= 1.0e-30 or volume_m3 <= 0.0:
        state.last_lubrication_pressure_center_pa = 0.0
        state.last_lubrication_pressure_radius_m = 0.0
        state.last_lubrication_pressure_force_n = 0.0
        state.last_lubrication_pressure_analytic_force_n = 0.0
        state.last_lubrication_pressure_effective_scale = _lubrication_pressure_effective_scale(state)
        state._lubrication_pressure_profile_cache = None
        return None

    # Reynolds squeeze-film pressure contribution used by the approach cases:
    #
    #   sigma_lub = -p_lub I
    #   F_lub = integral_Awet sigma_lub n dA
    #   p_lub(r) = C_lub * 3 mu |dh/dt| (a_lub^2 - r^2) / h^3
    #
    # This is the wall-pressure traction added in Cases 2-5.  It is motivated
    # by Reynolds lubrication theory, Reynolds, Phil. Trans. R. Soc. Lond. 177
    # (1886), pp. 188-190, Eqs. (17), (21), (24), and (27),
    # DOI: 10.1098/rstl.1886.0005.  The factor C_lub is reported separately
    # as lubrication_pressure_scale; Case 5 uses C_lub = 1.39.
    bridge_factor = 1.0 - (
        1.0 + 2.0 * volume_m3 / (math.pi * radius_m * gap_m * gap_m)
    ) ** -0.5
    bridge_factor = float(np.clip(bridge_factor, 0.0, 1.0))
    lubrication_radius_sq = max(radius_m * gap_m * bridge_factor, 0.0)
    lubrication_radius_m = math.sqrt(lubrication_radius_sq)
    pressure_scale = _lubrication_pressure_effective_scale(state)
    pressure_coeff = pressure_scale * (-3.0 * mu_pas * dhdt_mps / max(gap_m**3, 1.0e-30))
    pressure_center_pa = pressure_coeff * lubrication_radius_sq
    force_n = 0.5 * math.pi * pressure_coeff * lubrication_radius_sq * lubrication_radius_sq

    state.last_lubrication_pressure_center_pa = float(pressure_center_pa)
    state.last_lubrication_pressure_radius_m = float(lubrication_radius_m)
    state.last_lubrication_pressure_force_n = float(force_n)
    state.last_lubrication_pressure_analytic_force_n = float(force_n)
    state.last_lubrication_pressure_effective_scale = float(pressure_scale)
    profile = SimpleNamespace(
        ...
        lubrication_radius_sq_m2=float(lubrication_radius_sq),
        lubrication_radius_m=float(lubrication_radius_m),
        pressure_coeff_pa_per_m2=float(pressure_coeff),
        pressure_center_pa=float(pressure_center_pa),
        analytic_force_n=float(force_n),
    )
    state._lubrication_pressure_profile_cache = profile
    return profile


def _lubrication_pressure_at_point_pa(
    state: VolumetricPitoisState,
    point: np.ndarray,
    *,
    profile: SimpleNamespace,
) -> float:
    """Parabolic squeeze-film pressure evaluated from the current mesh radius."""
    config = state.config
    point_m = np.asarray(_length_from_compute(config, np.asarray(point, dtype=float)), dtype=float)
    bottom_center_m = np.asarray(
        _length_from_compute(config, np.asarray(state.bottom_sphere_center, dtype=float)),
        dtype=float,
    )
    axis = np.asarray(profile.axis, dtype=float)
    axis_norm = float(np.linalg.norm(axis))
    if axis_norm <= 1.0e-30 or not np.isfinite(axis_norm):
        return 0.0
    axis = axis / axis_norm
    rel = point_m - bottom_center_m
    radial = rel - float(np.dot(rel, axis)) * axis
    radial_sq = float(np.dot(radial, radial))
    pressure = float(profile.pressure_coeff_pa_per_m2) * max(
        float(profile.lubrication_radius_sq_m2) - radial_sq,
        0.0,
    )
...
def _gmsh_lubrication_pressure_force_vector_cache(
    state: VolumetricPitoisState,
    *,
    dim: int,
) -> dict[int, np.ndarray]:
    """Assemble squeeze-film pressure as a PR35 tetrahedral Cauchy force."""
    cache_by_key = getattr(state, "_gmsh_lubrication_pressure_force_cache", {})
    profile = _lubrication_pressure_profile(state)
    key = (
        int(dim),
        round(float(getattr(profile, "pressure_center_pa", 0.0)) if profile is not None else 0.0, 12),
        round(float(getattr(profile, "lubrication_radius_m", 0.0)) if profile is not None else 0.0, 12),
        round(float(getattr(profile, "pressure_scale", 0.0)) if profile is not None else 0.0, 12),
        round(float(np.asarray(getattr(state, "bottom_sphere_center", np.zeros(3)), dtype=float)[2]), 18),
        round(float(np.asarray(getattr(state, "top_sphere_center", np.zeros(3)), dtype=float)[2]), 18),
    )
    cached = cache_by_key.get(key)
    ...
    vertices = list(getattr(state, "volume_export_vertices", []))
    forces = np.zeros((len(vertices), int(dim)), dtype=float)
    if (
        profile is None
        or int(dim) != 3
        or state.HC is None
        ...
        force_map = {id(vertex): forces[idx] for idx, vertex in enumerate(vertices)}
        cache_by_key[key] = force_map
        state._gmsh_lubrication_pressure_force_cache = cache_by_key
        state.last_solver_lubrication_pressure_force_l1 = 0.0
        return force_map
    ...
    if np.any(valid):
        ...
        tet_volumes = np.asarray(volumes[valid], dtype=float)
        tet_grads = np.asarray(grads[valid], dtype=float)
        pressure_nodes_pa = np.asarray(
            [_lubrication_pressure_at_point_pa(state, point, profile=profile) for point in points],
            dtype=float,
        )
        pressure_nodes = np.asarray(
            _scale_to_compute(state.config, pressure_nodes_pa, "pressure_scale_pa"),
            dtype=float,
        )
        p_tet = np.mean(pressure_nodes[tet_nodes], axis=1)
        pressure_sigma = -p_tet[:, None, None] * np.eye(dim, dtype=float)[None, :, :]
        local_forces = -tet_volumes[:, None, None] * np.einsum("tab,tlb->tla", pressure_sigma, tet_grads)
        finite = np.all(np.isfinite(local_forces), axis=(1, 2))
        if np.any(finite):
            for local_idx in range(4):
                np.add.at(forces, tet_nodes[finite, local_idx], local_forces[finite, local_idx])

    state.last_solver_lubrication_pressure_force_l1 = float(np.sum(np.linalg.norm(forces, axis=1)))
    force_map = {id(vertex): np.asarray(forces[idx], dtype=float) for idx, vertex in enumerate(vertices)}
    cache_by_key[key] = force_map
    state._gmsh_lubrication_pressure_force_cache = cache_by_key
    return force_map


def _solver_lubrication_pressure_force(v, *, state: VolumetricPitoisState) -> np.ndarray:
    if not bool(
        getattr(
            state.config,
            "solver_add_lubrication_pressure_force",
        ...
        return np.zeros(3, dtype=float)
    force = _gmsh_lubrication_pressure_force_vector_cache(state, dim=3).get(id(v))
    if force is None:
        return np.zeros(3, dtype=float)
    return np.nan_to_num(np.asarray(force[:3], dtype=float), nan=0.0, posinf=0.0, neginf=0.0)


def _lubrication_pressure_wall_cap_force_physical(
    state: VolumetricPitoisState,
    *,
    which: str,
    profile: SimpleNamespace,
) -> np.ndarray:
    """Integrate sigma=-p(r)I over the wetted cap surface faces."""

    config = state.config
    if which == "bottom":
        tri_idx = np.asarray(getattr(state, "surface_export_bottom_cap_tris", []), dtype=int)
        sphere_center = np.asarray(state.bottom_sphere_center, dtype=float)
    else:
        tri_idx = np.asarray(getattr(state, "surface_export_top_cap_tris", []), dtype=int)
        sphere_center = np.asarray(state.top_sphere_center, dtype=float)
    vertices = list(getattr(state, "surface_export_vertices", []))
    if not vertices:
        vertices = list(getattr(state, "volume_export_vertices", []))
    if tri_idx.size == 0 or not vertices:
        setattr(state, f"last_lubrication_pressure_{which}_wall_faces", 0)
        setattr(state, f"last_lubrication_pressure_{which}_wall_active_faces", 0)
        setattr(state, f"last_lubrication_pressure_{which}_wall_area_m2", 0.0)
        setattr(state, f"last_lubrication_pressure_{which}_wall_active_area_m2", 0.0)
        return np.zeros(3, dtype=float)

    sphere_center_m = np.asarray(_length_from_compute(config, sphere_center), dtype=float)
    bottom_center_m, _top_center_m, axis = _particle_centers_physical(state)
    axis = np.asarray(axis, dtype=float)
    axis_norm = float(np.linalg.norm(axis))
    if axis_norm <= 1.0e-30 or not np.isfinite(axis_norm):
        return np.zeros(3, dtype=float)
    axis = axis / axis_norm
    axis_origin_m = np.asarray(bottom_center_m, dtype=float)
    pressure_coeff = float(profile.pressure_coeff_pa_per_m2)
    active_radius_sq = max(float(profile.lubrication_radius_sq_m2), 0.0)
    ...
    total_area = 0.0
    active_area = 0.0
    for tri in np.asarray(tri_idx, dtype=int).reshape((-1, 3)):
        a, b, c = (int(tri[0]), int(tri[1]), int(tri[2]))
        if min(a, b, c) < 0 or max(a, b, c) >= len(vertices):
            continue
        pa = np.asarray(_length_from_compute(config, np.asarray(vertices[a].x_a[:3], dtype=float)), dtype=float)
        pb = np.asarray(_length_from_compute(config, np.asarray(vertices[b].x_a[:3], dtype=float)), dtype=float)
        pc = np.asarray(_length_from_compute(config, np.asarray(vertices[c].x_a[:3], dtype=float)), dtype=float)
        area_vec = 0.5 * np.cross(pb - pa, pc - pa)
        area = float(np.linalg.norm(area_vec))
        if area <= 1.0e-30 or not np.isfinite(area):
            continue
        ...
        total_area += area
        centroid = (pa + pb + pc) / 3.0
        rel = centroid - axis_origin_m
        radial = rel - float(np.dot(rel, axis)) * axis
        radial_sq = float(np.dot(radial, radial))
        if radial_sq > active_radius_sq:
            continue
        normal = centroid - sphere_center_m
        normal_norm = float(np.linalg.norm(normal))
        if normal_norm <= 1.0e-30 or not np.isfinite(normal_norm):
            continue
        normal = normal / normal_norm
        pressure = pressure_coeff * max(active_radius_sq - radial_sq, 0.0)
        if not np.isfinite(pressure):
            continue
        # Liquid stress on the solid wall: sigma @ n with sigma=-pI.
        force += -float(pressure) * normal * area
        active_faces += 1
        active_area += area

    setattr(state, f"last_lubrication_pressure_{which}_wall_faces", int(total_faces))
    setattr(state, f"last_lubrication_pressure_{which}_wall_active_faces", int(active_faces))
    setattr(state, f"last_lubrication_pressure_{which}_wall_area_m2", float(total_area))
    setattr(state, f"last_lubrication_pressure_{which}_wall_active_area_m2", float(active_area))
    return np.nan_to_num(force, nan=0.0, posinf=0.0, neginf=0.0)


def _lubrication_pressure_cap_forces(state: VolumetricPitoisState) -> tuple[np.ndarray, np.ndarray]:
    """Return the gap-dependent squeeze-film pressure force on the two spheres.

    When requested, the force is integrated as a raw Cauchy wall traction
    ``sigma=-p(r)I`` over the wetted cap faces.  The closed-form resultant is
    kept as a fallback and diagnostic.
    """
    ...
    use_wall_traction = bool(
        getattr(
            config,
            "reported_gorge_lubrication_pressure_wall_traction",
            USER_REPORTED_GORGE_LUBRICATION_PRESSURE_WALL_TRACTION,
        )
    )
    if use_wall_traction:
        top_force_physical = _lubrication_pressure_wall_cap_force_physical(
            state,
            which="top",
            profile=profile,
        )
        bottom_force_physical = _lubrication_pressure_wall_cap_force_physical(
            state,
            which="bottom",
            profile=profile,
        )
        used_wall_faces = (
            int(getattr(state, "last_lubrication_pressure_top_wall_active_faces", 0)) > 0
            or int(getattr(state, "last_lubrication_pressure_bottom_wall_active_faces", 0)) > 0
        )
        if used_wall_faces:
            state.last_lubrication_pressure_force_n = float(np.dot(top_force_physical, axis))
        else:
            top_force_physical = float(profile.analytic_force_n) * axis
            bottom_force_physical = -float(profile.analytic_force_n) * axis
            state.last_lubrication_pressure_force_n = float(profile.analytic_force_n)
    else:
        top_force_physical = float(profile.analytic_force_n) * axis
        bottom_force_physical = -float(profile.analytic_force_n) * axis
        state.last_lubrication_pressure_force_n = float(profile.analytic_force_n)
    ...
    return (
        np.asarray(_scale_to_compute(config, top_force_physical, "force_scale_n"), dtype=float),
        np.asarray(_scale_to_compute(config, bottom_force_physical, "force_scale_n"), dtype=float),
    )
...
def _step_record(state: VolumetricPitoisState, *, step: int, t: float) -> dict[str, float | int]:
    ...
    top_pr33_cap_force = _solver_pr33_sphere_cap_force(state, which="top")
    bottom_pr33_cap_force = _solver_pr33_sphere_cap_force(state, which="bottom")
    ...
    top_cox_cl_out = np.asarray(
        _force_from_compute(state.config, sphere_forces.get("top_cox_contact_line", zero_force)),
        dtype=float,
    )
    bottom_cox_cl_out = np.asarray(
        _force_from_compute(state.config, sphere_forces.get("bottom_cox_contact_line", zero_force)),
        dtype=float,
    )
    ...
    top_wall_viscous_cap_out = np.asarray(
        _force_from_compute(state.config, sphere_forces.get("top_wall_viscous_cap", zero_force)),
        dtype=float,
    )
    bottom_wall_viscous_cap_out = np.asarray(
        _force_from_compute(state.config, sphere_forces.get("bottom_wall_viscous_cap", zero_force)),
        dtype=float,
    )
    top_wall_viscous_total_out = np.asarray(
        _force_from_compute(state.config, sphere_forces.get("top_wall_viscous_total", zero_force)),
        dtype=float,
    )
    bottom_wall_viscous_total_out = np.asarray(
        _force_from_compute(state.config, sphere_forces.get("bottom_wall_viscous_total", zero_force)),
        dtype=float,
    )
    top_pr33_wall_pressure_cap_out = np.asarray(
        _force_from_compute(state.config, sphere_forces.get("top_pr33_wall_pressure_cap", zero_force)),
        dtype=float,
    )
    bottom_pr33_wall_pressure_cap_out = np.asarray(
        _force_from_compute(state.config, sphere_forces.get("bottom_pr33_wall_pressure_cap", zero_force)),
        dtype=float,
    )
    top_pr33_wall_pressure_total_out = np.asarray(
        _force_from_compute(state.config, sphere_forces.get("top_pr33_wall_pressure_total", zero_force)),
        dtype=float,
    )
    bottom_pr33_wall_pressure_total_out = np.asarray(
        _force_from_compute(state.config, sphere_forces.get("bottom_pr33_wall_pressure_total", zero_force)),
        dtype=float,
    )
    top_lubrication_pressure_cap_out = np.asarray(
        _force_from_compute(state.config, sphere_forces.get("top_lubrication_pressure_cap", zero_force)),
        dtype=float,
    )
    bottom_lubrication_pressure_cap_out = np.asarray(
        _force_from_compute(state.config, sphere_forces.get("bottom_lubrication_pressure_cap", zero_force)),
        dtype=float,
    )
    top_lubrication_pressure_total_out = np.asarray(
        _force_from_compute(state.config, sphere_forces.get("top_lubrication_pressure_total", zero_force)),
        dtype=float,
    )
    bottom_lubrication_pressure_total_out = np.asarray(
        _force_from_compute(state.config, sphere_forces.get("bottom_lubrication_pressure_total", zero_force)),
        dtype=float,
    )
    ...
    top_pr33_cap_force_out = np.asarray(_force_from_compute(state.config, top_pr33_cap_force), dtype=float)
    bottom_pr33_cap_force_out = np.asarray(_force_from_compute(state.config, bottom_pr33_cap_force), dtype=float)
    return {
        "step": int(step),
        ...
        "top_cox_contact_line_force": top_cox_cl_out.tolist(),
        "bottom_cox_contact_line_force": bottom_cox_cl_out.tolist(),
        ...
        "top_cox_contact_line_axial": float(top_cox_cl_out[2]),
        "bottom_cox_contact_line_axial": float(bottom_cox_cl_out[2]),
        ...
        "reported_approach_solid_line_force": bool(
            getattr(
                state.config,
                "reported_approach_solid_line_force",
                USER_REPORTED_APPROACH_SOLID_LINE_FORCE,
        ...
        "reported_approach_solid_line_blend": float(
            _approach_solid_line_blend_weight(state)
            if bool(
                getattr(
                    state.config,
                    "reported_approach_solid_line_force",
        ...
        "top_wall_viscous_cap_traction": top_wall_viscous_cap_out.tolist(),
        "bottom_wall_viscous_cap_traction": bottom_wall_viscous_cap_out.tolist(),
        "top_wall_viscous_total_force": top_wall_viscous_total_out.tolist(),
        "bottom_wall_viscous_total_force": bottom_wall_viscous_total_out.tolist(),
        "top_wall_viscous_cap_axial": float(top_wall_viscous_cap_out[2]),
        "bottom_wall_viscous_cap_axial": float(bottom_wall_viscous_cap_out[2]),
        "top_wall_viscous_total_axial": float(top_wall_viscous_total_out[2]),
        "bottom_wall_viscous_total_axial": float(bottom_wall_viscous_total_out[2]),
        "top_pr33_wall_pressure_cap_traction": top_pr33_wall_pressure_cap_out.tolist(),
        "bottom_pr33_wall_pressure_cap_traction": bottom_pr33_wall_pressure_cap_out.tolist(),
        "top_pr33_wall_pressure_total_force": top_pr33_wall_pressure_total_out.tolist(),
        "bottom_pr33_wall_pressure_total_force": bottom_pr33_wall_pressure_total_out.tolist(),
        "top_pr33_wall_pressure_cap_axial": float(top_pr33_wall_pressure_cap_out[2]),
        "bottom_pr33_wall_pressure_cap_axial": float(bottom_pr33_wall_pressure_cap_out[2]),
        "top_pr33_wall_pressure_total_axial": float(top_pr33_wall_pressure_total_out[2]),
        "bottom_pr33_wall_pressure_total_axial": float(bottom_pr33_wall_pressure_total_out[2]),
        "top_lubrication_pressure_cap_traction": top_lubrication_pressure_cap_out.tolist(),
        "bottom_lubrication_pressure_cap_traction": bottom_lubrication_pressure_cap_out.tolist(),
        "top_lubrication_pressure_total_force": top_lubrication_pressure_total_out.tolist(),
        "bottom_lubrication_pressure_total_force": bottom_lubrication_pressure_total_out.tolist(),
        "top_lubrication_pressure_cap_axial": float(top_lubrication_pressure_cap_out[2]),
        "bottom_lubrication_pressure_cap_axial": float(bottom_lubrication_pressure_cap_out[2]),
        "top_lubrication_pressure_total_axial": float(top_lubrication_pressure_total_out[2]),
        "bottom_lubrication_pressure_total_axial": float(bottom_lubrication_pressure_total_out[2]),
        "lubrication_pressure_center_pa": float(
            getattr(state, "last_lubrication_pressure_center_pa", 0.0)
        ),
        "lubrication_pressure_radius_m": float(
            getattr(state, "last_lubrication_pressure_radius_m", 0.0)
        ),
        "lubrication_pressure_force_n": float(
            getattr(state, "last_lubrication_pressure_force_n", 0.0)
        ),
        "lubrication_pressure_analytic_force_n": float(
            getattr(state, "last_lubrication_pressure_analytic_force_n", 0.0)
        ),
        "lubrication_pressure_effective_scale": float(
            getattr(
                state,
                "last_lubrication_pressure_effective_scale",
                _lubrication_pressure_effective_scale(state),
            )
        ),
        "lubrication_pressure_top_wall_faces": int(
            getattr(state, "last_lubrication_pressure_top_wall_faces", 0)
        ),
        "lubrication_pressure_bottom_wall_faces": int(
            getattr(state, "last_lubrication_pressure_bottom_wall_faces", 0)
        ),
        "lubrication_pressure_top_wall_active_faces": int(
            getattr(state, "last_lubrication_pressure_top_wall_active_faces", 0)
        ),
        "lubrication_pressure_bottom_wall_active_faces": int(
            getattr(state, "last_lubrication_pressure_bottom_wall_active_faces", 0)
        ),
        "lubrication_pressure_top_wall_area_m2": float(
            getattr(state, "last_lubrication_pressure_top_wall_area_m2", 0.0)
        ),
        "lubrication_pressure_bottom_wall_area_m2": float(
            getattr(state, "last_lubrication_pressure_bottom_wall_area_m2", 0.0)
        ),
        "lubrication_pressure_top_wall_active_area_m2": float(
            getattr(state, "last_lubrication_pressure_top_wall_active_area_m2", 0.0)
        ),
        "lubrication_pressure_bottom_wall_active_area_m2": float(
            getattr(state, "last_lubrication_pressure_bottom_wall_active_area_m2", 0.0)
        ),
        ...
        "top_pr33_pressure_cap_force": top_pr33_cap_force_out.tolist(),
        "bottom_pr33_pressure_cap_force": bottom_pr33_cap_force_out.tolist(),
        "top_pr33_pressure_cap_axial": float(top_pr33_cap_force_out[2]),
        "bottom_pr33_pressure_cap_axial": float(bottom_pr33_cap_force_out[2]),
        ...
        "top_wall_viscous_faces": int(getattr(state, "last_gmsh_wall_viscous_top_faces", 0)),
        "bottom_wall_viscous_faces": int(getattr(state, "last_gmsh_wall_viscous_bottom_faces", 0)),
        "top_wall_viscous_matched_faces": int(
            getattr(state, "last_gmsh_wall_viscous_top_matched_faces", 0)
        ),
        "bottom_wall_viscous_matched_faces": int(
            getattr(state, "last_gmsh_wall_viscous_bottom_matched_faces", 0)
        ),
        "top_wall_viscous_area_m2": float(
            _area_from_compute(state.config, getattr(state, "last_gmsh_wall_viscous_top_area_m2", 0.0))
        ),
        "bottom_wall_viscous_area_m2": float(
            _area_from_compute(state.config, getattr(state, "last_gmsh_wall_viscous_bottom_area_m2", 0.0))
        ),
        "top_wall_viscous_matched_area_m2": float(
            _area_from_compute(state.config, getattr(state, "last_gmsh_wall_viscous_top_matched_area_m2", 0.0))
        ),
        "bottom_wall_viscous_matched_area_m2": float(
            _area_from_compute(state.config, getattr(state, "last_gmsh_wall_viscous_bottom_matched_area_m2", 0.0))
        ),
        ...
        "solver_pr33_free_surface_dirichlet_pressure": bool(
            getattr(state.config, "solver_pr33_free_surface_dirichlet_pressure", False)
        ),
        "solver_pr33_free_surface_dirichlet_cells": int(
            getattr(state, "last_solver_pr33_free_surface_dirichlet_cells", 0)
        ),
        ...
        "solver_add_lubrication_pressure_force": bool(
            getattr(
                state.config,
                "solver_add_lubrication_pressure_force",
                USER_SOLVER_ADD_LUBRICATION_PRESSURE_FORCE,
        ...
        "lubrication_pressure_scale": float(
            getattr(state.config, "lubrication_pressure_scale", USER_LUBRICATION_PRESSURE_SCALE)
        ),
        "solver_lubrication_pressure_force_l1_N": float(
            _force_from_compute(
                state.config,
                getattr(state, "last_solver_lubrication_pressure_force_l1", 0.0),
            )
        ),
        ...
        "gorge_report_pressure_scalar": float(
            _pressure_from_compute(
                state.config,
                getattr(state, "last_gorge_report_pressure_scalar", state.pressure_scalar),
            )
        ),
        "gorge_report_pr33_neck_pressure_delta": float(
            _pressure_from_compute(
                state.config,
                getattr(state, "last_gorge_report_pr33_neck_pressure_delta", 0.0),
            )
        ),
        "gorge_report_pr33_bridge_pressure_reference": float(
            _pressure_from_compute(
                state.config,
                getattr(state, "last_gorge_report_pr33_bridge_pressure_reference", 0.0),
            )
        ),
        "gorge_report_pr33_neck_pressure_samples": int(
            getattr(state, "last_gorge_report_pr33_neck_pressure_samples", 0)
        ),
        "pr33_free_surface_pressure_reference": float(
            _pressure_from_compute(
                state.config,
                getattr(state, "last_pr33_free_surface_pressure_reference", 0.0),
            )
        ),
        "pr33_free_surface_pressure_reference_samples": int(
            getattr(state, "last_pr33_free_surface_pressure_reference_samples", 0)
        ),
...
def _build_cli() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        choices=("axisym_fit", "axisym_initial", "fheron_neck", "axisym_fheron_floor", "axisym_initial_fheron_neck"),
    ...
    *,
    ...
    for step in range(config.n_steps):
        ...
        with _time_module(step_timing, "record"):
            record_interval = max(1, int(config.record_every))
            sparse_history_only = bool(getattr(config, "sparse_history_only", False))
            should_record_step = (
                verbose
                or (completed_step % record_interval == 0)
                or (sparse_history_only and completed_step == int(config.n_steps))
            )
            record_row = None
        ...
        with _time_module(step_timing, "record"):
            current_record_row = record_row
            if current_record_row is None and not bool(getattr(config, "sparse_history_only", False)):
                current_record_row = _step_record(
                    state,
                    step=completed_step,
                    t=state.elapsed_time_s,
                )
        if current_record_row is not None:
            history.append(current_record_row)
        progress_interval = max(
            1,
            int(config.record_every) if bool(getattr(config, "sparse_history_only", False)) else 500,
        )
        if save_results and completed_step % progress_interval == 0:
