...
def build_config(core, args):
    ...
    return replace(
        ...
        name=CASE_NAME,
        title=(
            "Pitois 2000 Fig. 6 approach: Case #4 raw force with "
            "smoothed contact-line motion and late-gap lubrication scaling"
        ),
        solver_add_lubrication_pressure_force=True,
        lubrication_pressure_scale=1.55,
        lubrication_pressure_scale_points=(
            # Preserve the attractive peak, then soften only the final squeeze
            # branch where the constant scale over-repels after the CL cap fix.
            (0.0500, 1.55),
            (0.0300, 1.55),
            (0.0250, 1.42),
            (0.0180, 1.27),
