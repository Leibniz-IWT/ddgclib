#!/usr/bin/env python3
"""Case #5: Fig. 6 approach with single-coefficient lubrication and CL smoothing."""
...
# Single global Reynolds-pressure coefficient. This replaces the old late-time
# D/R lookup table; cross-validation should use the same value unchanged.
LUBRICATION_PRESSURE_SCALE = 1.39
...
def build_config(core, args):
    ...
    motion_scaled_cl_slide_um = min(
        float(args.contact_line_max_slide_um),
        abs(float(args.approach_speed_um_s)) * float(args.dt),
    )
    return replace(
        config,
        name=CASE_NAME,
        title=(
            "Pitois 2000 Fig. 6 approach: Case #5 raw force with "
            "single-coefficient lubrication and motion-scaled contact-line smoothing"
        ),
        contact_line_max_slide_um=motion_scaled_cl_slide_um,
        solver_add_lubrication_pressure_force=True,
        lubrication_pressure_scale=LUBRICATION_PRESSURE_SCALE,
