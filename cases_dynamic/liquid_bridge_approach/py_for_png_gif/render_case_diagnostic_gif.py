...
except ModuleNotFoundError:  # pragma: no cover - separation helper not packaged here.
    CASE_DIRS = {}


ROOT = Path(__file__).resolve().parent
AXISYM_CHART_PATH = ROOT / "chart.py"
NEATPLOT_STYLE = ROOT / "standard.mplstyle"
...
CASE_LABELS = {
    ...
    "case13": "Case #13 force-calibrated mesh derived run",
}
...
def _load_axisym_chart_module():
    if not AXISYM_CHART_PATH.is_file():
        raise FileNotFoundError(
            f"{AXISYM_CHART_PATH} is not packaged with liquid_bridge_approach. "
            "The approach GIF scripts use this module only for shared drawing helpers."
        )
