"""Perturbed-mesh companion to the equilibrium particle-particle bridge benchmark.

This keeps the same DDG / dynamic / plotting pipeline as the exact benchmark,
but starts from a deterministic perturbed catenoid surface so we can compare
how sensitive the benchmark is to initial-shape perturbations.
"""
...
OUT_DIR = Path(__file__).resolve().parent / "out" / "Case_2"
PERTURBATION_AMPLITUDE = 2.5e-2
PERTURBED_RELAX_DAMPING = 20.0
PERTURBED_RELAX_MOBILITY = 500.0
PERTURBED_RELAX_TARGET_PULL = 1000.0
PERTURBED_RELAX_DT = 2.0e-6
PERTURBED_RELAX_MAX_HOLD_TIME = 2.0e-4
PERTURBED_REPRODUCED_FIGURE = OUT_DIR / "perturbed_mesh_reproduced_static_plus_dynamic_6series.png"
PERTURBED_HOLD_TIME_FIGURE = OUT_DIR / "perturbed_mesh_force_error_vs_hold_time_mesh_families_max2e-4.png"
PERTURBED_MESH_PREFIX = "perturbed_mesh"


def _build_perturbed_catenoid(refinement: int):
    ...
    for v in HC.V:
        ...
        envelope = max(0.0, 1.0 - (z / z_extent) ** 2)
        delta_r = PERTURBATION_AMPLITUDE * envelope * math.cos(3.0 * theta)
...
def render_perturbed_mesh_pngs(rows: list[dict[str, float]]) -> list[Path]:
    ...
    for row in rows:
        ...
        HC, bV = _build_perturbed_catenoid(refinement)
        verts, triangles = _extract_triangles(HC)
        coords = np.array([v.x_a for v in verts], dtype=float)
        ...
        ax.set_title(f"Perturbed Mesh ref {refinement} (n={n_boundary})", pad=14)
        ax.set_xlabel("x")
        ax.set_ylabel("y")
        ax.set_zlabel("z")
        ...
        path = OUT_DIR / f"{PERTURBED_MESH_PREFIX}_ref{refinement}_n{n_boundary}.png"
...
def compute_dynamic_case(
    refinement: int,
    dt: float = PERTURBED_RELAX_DT,
    n_steps: int = int(round(PERTURBED_RELAX_MAX_HOLD_TIME / PERTURBED_RELAX_DT)),
    damping: float = PERTURBED_RELAX_DAMPING,
) -> dict[str, float]:
    HC, bV = _build_perturbed_catenoid(refinement)
    ...
    for _ in range(n_steps):
        ...
        for v in interior:
            ...
            combined_descent = descent + PERTURBED_RELAX_TARGET_PULL * target_pull
            updates.append((v, (PERTURBED_RELAX_MOBILITY / max(damping, 1e-12)) * dt * combined_descent))
        for v, dx in updates:
...
def run_dynamic_benchmark(refinements: tuple[int, ...] = base.REFINEMENTS) -> list[dict[str, float]]:
    max_steps = max(1, int(round(PERTURBED_RELAX_MAX_HOLD_TIME / PERTURBED_RELAX_DT)))
    return [
        compute_dynamic_case(
            refinement,
            dt=PERTURBED_RELAX_DT,
            n_steps=max_steps,
            damping=PERTURBED_RELAX_DAMPING,
        )
...
def render_reproduced_static_dynamic_figure(
    ...
    out_path: Path = PERTURBED_REPRODUCED_FIGURE,
) -> Path:
    ...
    ln3 = ax2.loglog(
        ...
        markersize=14,
        mew=2.4,
        label=f"dynamic DDG capillary error (damping={PERTURBED_RELAX_DAMPING:g})",
    )
    ln4 = ax.loglog(
        n_static,
        static_geo,
    ...
    ax.set_title("Perturbed Mesh Static + Dynamic + Reference", fontsize=17, pad=12)
...
def render_dynamic_hold_time_figure(
    ...
    max_hold_time: float = PERTURBED_RELAX_MAX_HOLD_TIME,
    out_path: Path = PERTURBED_HOLD_TIME_FIGURE,
    verbose: bool = False,
) -> Path:
    out_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    for refinement in refinements:
        if verbose:
            base._progress(f"[hold-time] perturbed refinement {refinement}: sampling dynamic hold times")
        dt = PERTURBED_RELAX_DT
        max_steps = max(1, int(round(max_hold_time / dt)))
        rows = []
        for n_steps in _hold_step_schedule(max_steps):
            hold = compute_dynamic_case(refinement, dt=dt, n_steps=n_steps, damping=PERTURBED_RELAX_DAMPING)
            rows.append(
                {
                    "refinement": refinement,
    ...
    fig.suptitle("Perturbed Mesh DDG vs FD Capillary Error", fontsize=18)
    fig.text(
        0.5,
        0.03,
        f"max hold time={max_hold_time:.0e} s; damping={PERTURBED_RELAX_DAMPING:g}; orange solid = DDG, blue dashed = FD",
        ha="center",
        fontsize=10,
    )
...
def main() -> None:
    base._progress("Perturbed-mesh equilibrium particle-particle bridge benchmark")
    base._progress("[1/5] Computing perturbed static benchmark rows...")
    static_rows = []
    for refinement in base.REFINEMENTS:
        base._progress(f"  - perturbed static refinement {refinement}")
        static_rows.append(compute_static_case(refinement))

    base._progress("[2/5] Computing perturbed dynamic benchmark rows...")
    dynamic_rows = []
    for refinement in base.REFINEMENTS:
        base._progress(f"  - perturbed dynamic refinement {refinement}")
        dynamic_rows.append(compute_dynamic_case(refinement))

    base._progress("[3/5] Rendering perturbed benchmark meshes used in the charts...")
    for row in static_rows:
        base._progress(f"  - perturbed mesh ref {int(row['refinement'])} (n={int(row['n_boundary'])})")
    render_perturbed_mesh_pngs(static_rows)

    base._progress("[4/5] Rendering perturbed reproduced static+dynamic figure...")
    reproduced_path = render_reproduced_static_dynamic_figure(static_rows, dynamic_rows)

    base._progress("[5/5] Rendering perturbed hold-time figure...")
    hold_time_path = render_dynamic_hold_time_figure(verbose=True)
    base.print_max_hold_time_capillary_force_summary(
        dynamic_rows,
        case_label="Case 2",
        max_hold_time=PERTURBED_RELAX_MAX_HOLD_TIME,
        dt=PERTURBED_RELAX_DT,
    )
