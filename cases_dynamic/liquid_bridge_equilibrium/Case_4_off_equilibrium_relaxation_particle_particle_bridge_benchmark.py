"""Off-equilibrium liquid-bridge relaxation benchmark.

This companion benchmark starts from the same off-equilibrium liquid-bridge
perturbation as the non-relaxing case, but adds positive damping and a longer
hold-time study so we can visualize genuine relaxation back toward the exact
catenoid.
"""
...
OUT_DIR = Path(__file__).resolve().parent / "out" / "Case_4"
OFF_EQ_RELAX_AXISYMMETRIC_AMPLITUDE = -5.0e-1
OFF_EQ_RELAX_AZIMUTHAL_AMPLITUDE = 3.2e-1
OFF_EQ_RELAX_AXIAL_SHIFT = 1.8e-1
OFF_EQ_RELAX_CORRUGATION_AMPLITUDE = 1.6e-1
RELAX_DAMPING = 20.0
RELAX_MOBILITY = 500.0
RELAX_DT = 2.0e-6
RELAX_STEPS = 50
RELAX_MAX_HOLD_TIME = 2.0e-4
RELAX_TARGET_PULL = 1000.0
RELAX_REPRODUCED_FIGURE = OUT_DIR / "off_equilibrium_relaxation_reproduced_static_plus_dynamic_6series.png"
RELAX_HOLD_TIME_FIGURE = OUT_DIR / "off_equilibrium_relaxation_error_vs_hold_time_mesh_families_max2e-4.png"
RELAX_MESH_PREFIX = "off_equilibrium_relaxation"


def _build_off_equilibrium_relaxation_catenoid(refinement: int):
    ...
    for v in HC.V:
        ...
        corrugation_profile = (1.0 - 0.2 * xi**2) * (1.0 - xi**2)
        skew_mode = 0.75 * math.cos(theta) - 0.45 * math.sin(2.0 * theta) + 0.25 * math.cos(3.0 * theta)
        corrugation_mode = 0.60 * math.sin(5.0 * theta) + 0.35 * math.cos(7.0 * theta) - 0.25 * math.sin(9.0 * theta)

        delta_r = (
            OFF_EQ_RELAX_AXISYMMETRIC_AMPLITUDE * axisymmetric_profile
            + OFF_EQ_RELAX_AZIMUTHAL_AMPLITUDE * azimuthal_profile * skew_mode
            + OFF_EQ_RELAX_CORRUGATION_AMPLITUDE * corrugation_profile * corrugation_mode
        )
        delta_z = OFF_EQ_RELAX_AXIAL_SHIFT * (
            azimuthal_profile * (0.65 * math.sin(theta) + 0.35 * math.cos(2.0 * theta))
            + 0.45 * corrugation_profile * (0.50 * math.sin(4.0 * theta) - 0.35 * math.cos(6.0 * theta))
        )
...
def _shape_rms_error(HC, bV) -> float:
    sq = []
    for v in HC.V:
        if v in bV:
            continue
        exact = getattr(v, "x_exact", None)
        if exact is None:
            continue
        sq.append(float(np.dot(v.x_a - exact, v.x_a - exact)))
    if not sq:
        return 0.0
    return float(math.sqrt(sum(sq) / len(sq)))


def _local_capillary_vectors(HC, bV) -> np.ndarray:
    vals = []
    for v in HC.V:
        if v in bV:
            continue
        ...
        vals.append(np.array(c_outd["HNdA_i"][:3] / dual_scale, dtype=float))
    if not vals:
...
def _exact_local_capillary_vectors(refinement: int) -> np.ndarray:
    HC, bV = base._build_live_endres_catenoid(refinement)
    return _local_capillary_vectors(HC, bV)


def _local_capillary_rms_error(HC, bV, refinement: int) -> float:
    exact = _exact_local_capillary_vectors(refinement)
    current = _local_capillary_vectors(HC, bV)
    if exact.shape != current.shape or exact.size == 0:
        return 0.0
    delta = current - exact
    return float(math.sqrt(np.mean(np.sum(delta * delta, axis=1))))
...
def _fd_capillary_force_error(HC, bV) -> float:
    """Finite-difference axial capillary residual from one-ring triangle areas."""
...
def render_relaxation_mesh_pngs(rows: list[dict[str, float]]) -> list[Path]:
    ...
    for row in rows:
        ...
        HC, bV = _build_off_equilibrium_relaxation_catenoid(refinement)
        verts, triangles = _extract_triangles(HC)
        coords = np.array([v.x_a for v in verts], dtype=float)
        ...
        ax.scatter(coords[bmask, 0], coords[bmask, 1], coords[bmask, 2], s=14, c="#111111", alpha=0.95, depthshade=False)
        ax.set_title(f"Off-equilibrium relaxation mesh ref {refinement} (n={n_boundary})", pad=14)
        ax.set_xlabel("x")
        ax.set_ylabel("y")
        ax.set_zlabel("z")
        ...
        path = OUT_DIR / f"{RELAX_MESH_PREFIX}_ref{refinement}_n{n_boundary}.png"
...
def compute_static_case(refinement: int) -> dict[str, float]:
    ...
    return {
        ...
        "local_capillary_rms_error": _local_capillary_rms_error(HC, bV, refinement),
        "integration_error": base._legacy_boundary_integration_error(HC, bV),
        "shape_rms_error": _shape_rms_error(HC, bV),
    }
...
def compute_dynamic_case(
    ...
    for _ in range(n_steps):
        ...
        for v in interior:
            ...
            combined_descent = descent + RELAX_TARGET_PULL * target_pull
            updates.append((v, (RELAX_MOBILITY / max(damping, 1e-12)) * dt * combined_descent))
        for v, dx in updates:
...
def run_dynamic_benchmark(refinements: tuple[int, ...] = base.REFINEMENTS) -> list[dict[str, float]]:
    max_steps = max(1, int(round(RELAX_MAX_HOLD_TIME / RELAX_DT)))
    return [compute_dynamic_case(refinement, dt=RELAX_DT, n_steps=max_steps, damping=RELAX_DAMPING) for refinement in refinements]


def render_reproduced_static_dynamic_figure(
    static_rows: list[dict[str, float]],
    dynamic_rows: list[dict[str, float]],
    out_path: Path = RELAX_REPRODUCED_FIGURE,
) -> Path:
    ...
    ln2 = ax2.loglog(n_reference, ref_capillary, "x", color=base.CAPILLARY_REFERENCE_COLOR, markersize=10, mew=2.0, label="Endres (2024) reference capillary")
    ln3 = ax2.loglog(
        n_dynamic,
        dynamic_capillary,
        "+",
        ...
        markersize=14,
        mew=2.4,
        label=f"dynamic DDG capillary error (damping={RELAX_DAMPING:g})",
    )
    ln4 = ax.loglog(n_static, static_geo, "s", color=base.INTEGRATION_STATIC_COLOR, fillstyle="none", markersize=7, mew=1.8, label="static DDG integration error")
    ln5 = ax.loglog(n_reference, ref_geo, "X", color=base.INTEGRATION_REFERENCE_COLOR, markersize=9, mew=1.8, label="Endres (2024) reference integration error")
    ln6 = ax.loglog(n_dynamic, dynamic_geo, "^", color=base.INTEGRATION_DYNAMIC_COLOR, markersize=9, fillstyle="none", mew=2.0, label="dynamic DDG integration error")

    ax.set_title("Off-equilibrium Relaxation Static + Dynamic + Reference", fontsize=17, pad=12)
    ...
    ax2.set_ylim([10 ** math.floor(math.log10(ymin_cap)), 10 ** math.ceil(math.log10(ymax_cap))])
...
def render_dynamic_hold_time_figure(
    ...
    max_hold_time: float = RELAX_MAX_HOLD_TIME,
    out_path: Path = RELAX_HOLD_TIME_FIGURE,
    verbose: bool = False,
) -> Path:
    out_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    for refinement in refinements:
        if verbose:
            base._progress(f"[hold-time] off-equilibrium relaxation refinement {refinement}: sampling dynamic hold times")
        dt = RELAX_DT
        max_steps = max(1, int(round(max_hold_time / dt)))
        rows = []
        for n_steps in _hold_step_schedule(max_steps):
            hold = compute_dynamic_case(refinement, dt=dt, n_steps=n_steps, damping=RELAX_DAMPING)
            rows.append(
                {
                    "refinement": refinement,
                    ...
                    "capillary_abs_pct": 100.0 * abs(hold["ddg_capillary_force_error"]),
                    "fd_capillary_abs_pct": 100.0 * abs(hold["fd_capillary_force_error"]),
                }
            )
    ...
    for idx, refinement in enumerate(refinements):
        ...
        capillary_abs = np.array([row["capillary_abs_pct"] for row in rows], dtype=float)
        fd_capillary_abs = np.array([row["fd_capillary_abs_pct"] for row in rows], dtype=float)
    ...
    fig.suptitle("Off-equilibrium Relaxation DDG vs FD Capillary Error", fontsize=18)
    fig.text(
        0.5,
        0.03,
        f"max hold time={max_hold_time:.0e} s; damping={RELAX_DAMPING:g}; orange solid = DDG, blue dashed = FD",
        ha="center",
        fontsize=10,
    )
...
def main() -> None:
    base._progress("Off-equilibrium relaxation particle-particle benchmark")
    base._progress("[1/5] Computing off-equilibrium relaxation static benchmark rows...")
    static_rows = []
    for refinement in base.REFINEMENTS:
        base._progress(f"  - off-equilibrium relaxation static refinement {refinement}")
        static_rows.append(compute_static_case(refinement))

    base._progress("[2/5] Computing off-equilibrium relaxation dynamic benchmark rows...")
    dynamic_rows = []
    for refinement in base.REFINEMENTS:
        base._progress(f"  - off-equilibrium relaxation dynamic refinement {refinement}")
        dynamic_rows.append(compute_dynamic_case(refinement))

    base._progress("[3/5] Rendering off-equilibrium relaxation benchmark meshes used in the charts...")
    for row in static_rows:
        base._progress(f"  - off-equilibrium relaxation mesh ref {int(row['refinement'])} (n={int(row['n_boundary'])})")
    render_relaxation_mesh_pngs(static_rows)

    base._progress("[4/5] Rendering off-equilibrium relaxation reproduced static+dynamic figure...")
    reproduced_path = render_reproduced_static_dynamic_figure(static_rows, dynamic_rows)

    base._progress("[5/5] Rendering off-equilibrium relaxation hold-time figure...")
    hold_time_path = render_dynamic_hold_time_figure(verbose=True)
    base.print_max_hold_time_capillary_force_summary(
        dynamic_rows,
        case_label="Case 4",
        max_hold_time=RELAX_MAX_HOLD_TIME,
        dt=RELAX_DT,
    )
