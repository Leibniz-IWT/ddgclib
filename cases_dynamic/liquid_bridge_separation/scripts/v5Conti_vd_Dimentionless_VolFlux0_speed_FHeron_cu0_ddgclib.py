...
Equation/reference map used by this solver:
    ...
    PR33 pressure force:
        F_p = B^T p is an internal PR33/ddgclib operator path.  It is available
        in this solver, but cases 1-8 call this file with PR33 pressure force
        disabled.
...
# PR33 operator dependency available for cases that enable the pressure-force
# path.  The PR archive keeps a local copy beside this file, with a fallback to
# the original ddgclib package path when this solver is run from the full repo.
...
USER_ENABLE_SOLVER_HYDROSTATIC_FORCE = False
USER_SOLVER_HYDROSTATIC_ZREF_MODE = "midpoint"
USER_ENABLE_SOLVER_PR33_PRESSURE_FORCE = False
...
USER_PRESSURE_CLOSURE = "incompressible"
USER_COMPRESSIBLE_BULK_MODULUS_PA = 1.0e5
USER_COMPRESSIBLE_USE_INCOMPRESSIBLE_LIMIT_PROJECTION = False
USER_COMPRESSIBLE_EOS_POSITION_CORRECTION_RELAXATION = 0.25
USER_COMPRESSIBLE_EOS_PRESSURE_DELTA_LIMIT_FRACTION = 0.0
...
class VolumetricPitoisConfig:
    ...
    solver_viscous_force_model: str = "edge_flux"
    ...
    solver_pr33_pressure_reference_mode: str = "heron"
...
def separation_config() -> VolumetricPitoisConfig:
    return VolumetricPitoisConfig(
        name="v5Conti_vd_Dimentionless_VolFlux0_speed_v3",
        title="Case 2b axisym: dimensionless VolFlux0_v3 with Gmsh vd stress speed path",
    )
...
def _advance_gmsh_incompressible_substep(
    ...
    *,
    ...
    if (
        closure == "compressible"
        and not bool(getattr(state.config, "enable_solver_pr33_pressure_force", False))
        and not bool(getattr(state.config, "compressible_use_incompressible_limit_projection", False))
    ):
        return False
    if (
        (not bool(getattr(state, "gmsh_compute_mesh", False)))
...
def _build_cli() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        dest="use_pitois_eq6_contact_radius",
        action="store_true",
        default=None,
        help="Use the Pitois Eq. [6] contact-radius target while keeping the loaded initial mesh.",
    )
    parser.add_argument(
        "--no-use-pitois-eq6-contact-radius",
        dest="use_pitois_eq6_contact_radius",
        action="store_false",
        help="Do not use the Eq. [6] contact-radius target when the loaded mesh is used.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        dest="allow_contact_line_growth",
        action="store_true",
        default=USER_ALLOW_CONTACT_LINE_GROWTH,
        help="Allow the contact-line radius to increase during contact-angle kinematics.",
    )
    parser.add_argument(
        "--no-allow-contact-line-growth",
        dest="allow_contact_line_growth",
        action="store_false",
        help="Prevent contact-line radius growth during contact-angle kinematics.",
    )
    parser.add_argument(
...
def main(argv: list[str] | None = None) -> None:
    ...
    use_pitois_eq6_contact_radius = initial_contact_radius_mode != "loaded"
    if args.use_pitois_eq6_contact_radius is not None:
        use_pitois_eq6_contact_radius = bool(args.use_pitois_eq6_contact_radius)
    ...
    config = replace(
        ...
        enable_incompressible_projection=(str(args.closure) == "incompressible"),
        ...
        use_pitois_eq6_contact_radius=use_pitois_eq6_contact_radius,
        ...
        allow_contact_line_growth=bool(args.allow_contact_line_growth),
