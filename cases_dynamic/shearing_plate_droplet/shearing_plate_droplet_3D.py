#!/usr/bin/env python3
"""3D shearing-plate droplet simulation.

Oil droplet suspended in water inside a cuboidal channel with
plates at ``y = ±L_y`` counter-moving at ``±U_wall * e_x``.  The
streamwise (x) and spanwise (z) directions are periodic.

Outputs (in this directory):
  fig/shearing_plate_droplet_3D_fluid.png       — final pressure + velocity
  fig/shearing_plate_droplet_3D_profile.png     — mean u_x(y) vs Couette
  fig/shearing_plate_droplet_3D_deformation.png — D(t), tilt(t)
  fig/shearing_plate_droplet_3D.mp4             — 3D animation
  results_3d/snapshots/                          — JSON snapshots
...
from cases_dynamic.shearing_plate_droplet.src._params import (
    ...
    t_end_default, Ca, visc_ratio, Re, shear_rate,
)
...
def main():
    dim = 3
    print("=" * 64)
    print("3D Shearing-Plate Droplet")
    print("=" * 64)
    ...
    t_end = 0.5 * t_end_default   # 3D is expensive; shorter window
    ...
    try:
        ...
        fig, ax = plot_velocity_profile(
            HC, U_wall=U_wall, L_y=L_y, dim=dim, n_bins=20,
            title=f'3D Mean $u_x(y)$ at t = {t_final:.3f} s',
            save_path=os.path.join(_FIG,
                                    'shearing_plate_droplet_3D_profile.png'),
        )
        plt.close(fig)
        ...
        fig, _ = plot_deformation_history(
            ...
            save_path=os.path.join(
                _FIG, 'shearing_plate_droplet_3D_deformation.png'),
        )
        plt.close(fig)
    ...
    if history.n_snapshots > 1:
        try:
            ...
            print(f"Animation saved to fig/shearing_plate_droplet_3D.mp4")
