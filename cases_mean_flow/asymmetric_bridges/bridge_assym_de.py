...
if 1:
    ...
    ln1 = ax.loglog(Nlist, errors_point_wise_1*100, 'x', color='tab:blue',
                     label=r'Punktweise Fehler in $\mathbf{x_{+}}=(0, 0, 1)$ Richtung  ($\frac{\widehat{H\mathbf{N}} d A_{i} \cdot \mathbf{x_{+}}}{C_i}$)')
    ln2 = ax.loglog(Nlist, errors_point_wise_2*100, 'X', color='tab:red',
                     label=r'Punktweise Fehler in $\mathbf{x_{-}}=(0, 0, -1)$ Richtung ($\frac{\widehat{H\mathbf{N}} d A_{i} \cdot \mathbf{x_{-}}}{C_i}$)')

    #ax2.set_ylabel(r'Young-Laplace Fehler (%)')
    ax.set_xlabel(r'$n$ (Anzahl der Scheitelpunkte)')

    ln3 = ax.loglog(Nlist,  np.abs(np.sum(errors_total, axis=1))*100, 'o', color='tab:orange',
                    label=r'Integrierte Fehler (Summe der Vektorkomponenten in $\widehat{H\mathbf{N}} d A_{i}$ )')
    ax.set_ylabel(f'Integrationsfehler (%)')
