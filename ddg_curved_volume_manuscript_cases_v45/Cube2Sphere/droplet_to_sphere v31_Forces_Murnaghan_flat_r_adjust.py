...
def mean_curvature_scalar(xyz, faces):
    ...
    return Hn, N, Astar, W, Lx   # <---- CHANGED: also return Lx
...
def F_surface_tension(sigma, Hn, N, Astar):
    # With outward normals and convex geometry, Hn < 0 -> inward force.
...
script_stem = Path(__file__).stem # e.g. droplet_to_sphere_modular_forces_EOS
OUTPUT_DIR = script_dir / script_stem # folder name == py name
...
def step_forces_dualvolume(HC, faces, step_idx: int):
    ...
    # CHANGED: get Lx from curvature routine
    ...
    # --------- NEW: k=0 mean-jump initialization (minimal change) ---------
    ...
    # CHANGED: full shape-derivative capillary force (no projection)
    Fs = SIGMA * Lx
    ...
    # --------- (NEW) tiny radial projection blend toward sphere of volume REF_VOLUME ---------
...
# Read rel.error columns from the summary CSV we just wrote
import pandas as pd
