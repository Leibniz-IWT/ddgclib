...
USE_CURVED_VOLUME = True  # set to False for pure PL run
...
class _CVTempDir(tempfile.TemporaryDirectory):
    def __init__(self, *a, **kw):
        base_dir = globals().get("OUTPUT_DIR", Path(tempfile.gettempdir()))
        tmp_root = Path(base_dir) / "_cv_tmp"
        tmp_root.mkdir(parents=True, exist_ok=True)
        kw.setdefault("dir", str(tmp_root))
        super().__init__(*a, **kw)
...
print(f"Total computing time: {total_wall_s:.6f} s")
