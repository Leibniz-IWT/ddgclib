...
INERTIA_SCALE = 1.5   # l=2 effective interface-only inertia
...
if USE_CONSISTENT_MASS:
    ...
    MASS_MATRIX = None
    VI_REF       = Vi_true0
    MASS_I_VEC   = INERTIA_SCALE * RHO0 * VI_REF  # initial masses (may be updated if UPDATE_MASS_EACH_STEP=True)
    MASS_TOTAL   = float(np.sum(MASS_I_VEC))
...
def step_forces_dualvolume(HC_unused, faces, step_idx: int):
    ...
    Astar_safe = np.maximum(Aflat_pt, 0)
    ...
    if UPDATE_MASS_EACH_STEP and (not USE_CONSISTENT_MASS):
        if FIX_VI_AT_T0:
            ...
            MASS_I_VEC = INERTIA_SCALE * RHO0 * Vi_true_plus
        MASS_TOTAL = float(np.sum(MASS_I_VEC))
    ...
    if (step_idx % SAVE_EVERY) == 0:
        ...
        mm = 1.0e-3
        ax.set_xlim(-1.0 * mm,  1.0 * mm)
        ax.set_ylim(-1.0 * mm,  1.0 * mm)
        ax.set_zlim(-1.0 * mm,  1.0 * mm)
...
with open(summary_csv, "w", newline="") as f:
    ...
    for (
        ...
        rel_error_V = ((Vi_true_plus_total - REF_VOLUME) / max(REF_VOLUME, 1e-300)) * 100.0
...
# ===================== NEW: plot styling knobs (requested) =====================
LINE_W = 1.5
AXLABEL_SCALE = 1.2  # +20%

def bump_axis_label_fonts(ax, scale=1.2):
    # call AFTER set_xlabel/set_ylabel
    ax.xaxis.label.set_size(ax.xaxis.label.get_size() * scale)
    ax.yaxis.label.set_size(ax.yaxis.label.get_size() * scale)
# ==============================================================================
...
# Left: rel.error.A%
ax[0].plot(iters_csv, rel_err_A, linewidth=LINE_W)
ax[0].set_title("rel.error.A%")
ax[0].set_xlabel("iter")
ax[0].set_ylabel("%")
bump_axis_label_fonts(ax[0], AXLABEL_SCALE)
...
# Right: rel.error.V%
ax[1].plot(iters_csv, rel_err_V, linewidth=LINE_W)
ax[1].set_title("rel.error.V%")
ax[1].set_xlabel("iter")
ax[1].set_ylabel("%")
bump_axis_label_fonts(ax[1], AXLABEL_SCALE)
...
ax2.plot(t_dimless, A_dimless, linewidth=LINE_W)
ax2.set_xlabel(r"Dimensionless time $t^* = f_R t$")
ax2.set_ylabel(r"$A(t) / A_{\mathrm{ref}}$")
bump_axis_label_fonts(ax2, AXLABEL_SCALE)
...
#ax3.plot(t_dimless, A_dimless_rescaled, linewidth=LINE_W)
...
#bump_axis_label_fonts(ax3, AXLABEL_SCALE)
