...
def _build_volume_csv_by_routing(tr_csv_path: Path) -> Path:
    ...
    if extra_cols:
        ...
        out_df = out_df.merge(df_ids, on="triangle_id", how="left")
...
def _write_dualvolume_csv_from_vol_csv(vol_csv: Path) -> Path:
    ...
    Rule for DualVolume:
      - if PointID == A_id -> add V_patch_A
      - if PointID == B_id -> add V_patch_B
      - if PointID == C_id -> add V_patch_C
      - PointID can appear multiple times across triangles; sum them

    Rule for DualArea:
      - if PointID == A_id -> add A_patch_A
      - if PointID == B_id -> add A_patch_B
      - if PointID == C_id -> add A_patch_C
      - same summation logic as volume

    NEW: if the original *_COEFFS.csv has, for that triangle_id,
         Max_Residual_ABC > Residual_Threshold,
         then ALL 6 patch quantities of that triangle (V_patch_A/B/C and A_patch_A/B/C)
         are ignored (treated as 0).

    NEW: we also propagate the Type ("ellipsiod", "rotation", "translation") per point.
          If a point gets several types, we keep the first one we see.
    """
    vol_csv = Path(vol_csv).resolve()
    ...
    for pid in all_pids:
        rows.append({
            "PointID": pid,
            "DualVolume": acc_vol.get(pid, 0.0),
            "DualArea": acc_area.get(pid, 0.0),
