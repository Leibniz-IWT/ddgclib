...
- Writes:
    ...
    <mesh>_RINGS.csv        (per-triangle ring1/ring2 vertex IDs)
    <mesh>_COEFFS.csv       (per-triangle quadric coefficients)
    <mesh>_Tri_TYPE.csv     (per-triangle: plane/curved + max_d + thr)
...
def rescale_smart(coeff: np.ndarray, tol_abs: float = 1e-9, tol_rel: float = 1e-6, prefer: str = "H") -> np.ndarray:
    ...
    if abs(J) > thr:
        return c * (-1.0 / J)  # set J == -1 for non-paraboloids

    if prefer.upper() == "H" and abs(H) > thr:
        s = 1.0 / H            # fallback: make H == +1
        return c * s
...
def _is_vertical_paraboloid_like(c: np.ndarray,
    ...
    z-axis paraboloid pattern (1× convention):
      A≈B, C≈D≈E≈F≈G≈H≈0, and I≠0.
      Typical:  A x^2 + B y^2 + I z + J = 0
    """
...
def _canonize_paraboloid_A1(c: np.ndarray,
    ...
    B snaps to 1 if extremely close; J is preserved.
    """
    c = np.array(c, float)
...
def fit_quadric_stable(P: np.ndarray, irls_iters: int = 0, snap_rel: float = 5e-6, snap_abs: float = 1e-9) -> np.ndarray:
    ...
    if _is_vertical_paraboloid_like(c):
        c = _canonize_paraboloid_A1(c)   # preserve J
    else:
        c = rescale_smart(c)             # may set J -> -1 (or H -> +1)
...
def build_vertex_adjacency_smooth(points: np.ndarray, tris: np.ndarray, theta_max_deg: float):
    ...
    for (u,v), tids in edge_map.items():
        smooth = False
        if len(tids) < 2:
            smooth = False  # boundary edge is NOT smooth
        else:
...
def rim_vertices_by_dihedral(points: np.ndarray, tris: np.ndarray, theta_max_deg: float) -> np.ndarray:
    """
    Rim = (per-vertex max dihedral angle) > theta_max_deg.
    Uses only dihedral angle. No special-casing for boundaries.
    """
    v_max_dih = vertex_max_dihedral_deg(points, tris)  # degrees; boundary edges will contribute up to 180
...
def _k_ring_topological(G: List[Set[int]], seed: int, min_pts: int = 10, max_k: int = 4) -> np.ndarray:
    """Expand topological k-rings from seed until we reach >= min_pts or hit max_k."""
    patch = {seed}
    frontier = {seed}
    k = 0
...
def classify_points_and_write_csv(points: np.ndarray, tris: np.ndarray, rim_depth: np.ndarray, adj_smooth,
                                  ...
                                  plane_angle_deg: float = 5.0,  # unused now (kept for CLI compat)
                                  face_role_triplets: Optional[Tuple[np.ndarray,np.ndarray,np.ndarray]] = None,
                                  G_topo: Optional[List[Set[int]]] = None):
    """
    Writes: point_id,type,rim,plane,surfacepoint,d_plane,dihedral_angle,
            on_boundary_face,on_interior_face,on_nonmanifold_face
    """
    nV = points.shape[0]
    ...
    for v in range(nV):
        ...
        if sp:
            ...
            if idx_used.size >= 3:
                ...
                plane_flag = True  # tiny neighborhood -> treat as planar
...
def adaptive_patch_in_component(
    ...
    min_pts: int = 10,
    adj_smooth: Optional[List[Set[int]]] = None,  # kept for signature parity
    max_rings: int = 4,
    global_rings: int = 4,
    G_topo: Optional[List[Set[int]]] = None,      # <- pass in a topological graph
) -> np.ndarray:
    """
    Build a vertex patch around `seed` inside smooth component `comp`.

    Rim-safe rule: if the seed is on the rim (rim-depth==0), only accept
    neighbors whose rim-depth is >= 1 (moving inward).

    Phase 1: grow inside the given component via `adj_comp`.
    Phase 2: (optional) global topological fallback via `G_topo`.
    """
    import numpy as _np
    ...
    # ---- phase 1: grow within the smooth component ----
    rings = 0
    ...
    # ---- phase 2: global fallback using provided topological graph ----
...
def compute_rimsafe_alltri(
    ...
    plane_rel_tol: float = 1e-3,
    plane_abs_tol: float = 1e-9,
    plane_angle_deg: float = 5.0,  # kept for CLI, not used in plane-only classifier
    irls_iters: int = 0,
    snap_rel: float = 5e-6,
    snap_abs: float = 1e-9,
    ...
    # Resolve the single-source default
    ...
    # --- sanity check for a vertex (115 kept for continuity) ---
    vid = 115
    v_max_dih_all = vertex_max_dihedral_deg(points, tris)  # degrees
    ...
    # Build topological graph once (used for POINT_TYPES & patch fallback)
    G_topo = _vertex_graph_from_tris(len(points), tris)

    # -------- Per-vertex classification CSV (PLANE ONLY) --------
    ...
    _, nonplanar_mask = classify_points_and_write_csv(
        ...
        plane_angle_deg=plane_angle_deg,  # ignored in this mode
        face_role_triplets=(on_boundary_face, on_interior_face, on_nonman_face),
        G_topo=G_topo,
    )

    # -------- Per-triangle Ring CSV (for debugging) --------
    ...
    # -------- Per-triangle TYPE CSV (plane/curved + max_d + thr) --------
    tri_type_rows = []

    # -------- Quadric regression CSV --------
    rows = []; cols_coeff = list("ABCDEFGHIJ"); skipped_flat = 0
    TARGET_TRI = 208  # debug triangle (kept)
    ...
    for tri_id, (a,b,c) in enumerate(tris):
        ...
        # ABC union then FILTER to non-planar types only (based on PLANE-ONLY classifier)
        idxABC = np.unique(np.concatenate([idxA, idxB, idxC]))
        if idxABC.size:
            idxABC = idxABC[nonplanar_mask[idxABC]]

        # ---- DEBUG for a specific triangle ----
        if tri_id == TARGET_TRI:
            ...
            print(f"[DEBUG T{tri_id}] A IDs :", idxA.tolist())
            print(f"[DEBUG T{tri_id}] B IDs :", idxB.tolist())
            print(f"[DEBUG T{tri_id}] C IDs :", idxC.tolist())
            print(f"[DEBUG T{tri_id}] ABC IDs:", idxABC.tolist())
            print(f"[DEBUG T{tri_id}] ABC rim-depths:", [int(rd[v]) for v in idxABC])

        # ABC planarity gate + capture diagnostics
        ...
        # ---- record tri type BEFORE any continue ----
    ...
    # ---- write triangle TYPE CSV ----
    out_tri_type_csv = base + "_Tri_TYPE.csv"
    pd.DataFrame(tri_type_rows, columns=["triangle_id","type","max_d","thr"]).to_csv(out_tri_type_csv, index=False)
