...
def quadric_major_z_minor_x(coeffs, tol=1e-8, offdiag_rel=1e-3):
    ...
    # --- NEW: near-diagonal snap to kill tiny off-diagonals (prevents eigen-rotate) ---
    base = max(abs(A), abs(B), abs(C), 1.0)
    if max(abs(D), abs(E), abs(F)) <= offdiag_rel * base:
        # treat as diagonal; no further rotation
        return (A, B, C, 0.0, 0.0, 0.0, G, H, I_lin, J), R_acc

    # --- original small-absolute off-diagonal check (kept) ---
    tol_off = tol * base + tol
    if abs(D) <= tol_off and abs(E) <= tol_off and abs(F) <= tol_off:
        return (A, B, C, 0.0, 0.0, 0.0, G, H, I_lin, J), R_acc

    # --- eigen path (unchanged except chaining R_acc) ---
...
def analyse_quadric(*coeffs: float,
    ...
    print("!!!!debug:input coeffs:", coeffs_org)
    ...
    # linear and constant terms after center translation
    ...
    # === Change #1: zero linear terms only if they cancel numerically after the translation
    if abs(G1) <= SNAP_TOL: G1 = 0.0
    if abs(H1) <= SNAP_TOL: H1 = 0.0
    if abs(I1) <= SNAP_TOL: I1 = 0.0

    # 3) Optional: rotate inside the (near) nullspace AFTER translation to collapse (H1,I1)->(±||·||,0)
    a1, b1, c1 = _A, _B, _C
    R_null = np.eye(3)
    zero_mask = [abs(a1) <= SNAP_TOL, abs(b1) <= SNAP_TOL, abs(c1) <= SNAP_TOL]
    if sum(zero_mask) >= 2:
        #print("####yes#####")
        # pick a 2D plane of (almost) zero curvature
        ...
        if nrm > SNAP_TOL:
            # rotate so second component becomes 0 and first becomes nrm
            c = u[0] / nrm
            s = u[1] / nrm
            R2 = np.eye(3)
    ...
    # === Change #2: if we still have J1 != 0 and there is a pure-linear (parabolic) axis,
    # shift along that axis to make J -> 0 without altering linear terms.
    # (Along a null axis, A*delta = 0 and cross-terms are zero in this frame.)
    a1, b1, c1 = _A, _B, _C
    if abs(J1) > SNAP_TOL:
    ...
    def choose_s(a, lin, j, targetJ):
        ...
        if targetJ == 0.0:
            ...
            # target J -> -1: pick a scale that makes (A or linear) match J magnitude
            if abs(a) > SNAP_TOL:
                val = (-j)/abs(a)
    ...
    G2 = g1 * sx;       H2 = h1 * sy;       I2 = i1 * sz;       J2 = j1

    # global normalization for J
    if targetJ == -1.0 and abs(J2) > J_FORCE_THR:
        alpha = -1.0 / J2
    else:
        alpha = 1.0
    ...
    def sgn_unit(v): return 0.0 if abs(v) <= 1e-8 else (1.0 if v > 0 else -1.0)
    if abs(a1) > SNAP_TOL: A3 = sgn_unit(A3)
    else:                  A3, G3 = 0.0, (math.copysign(1.0, G3) if abs(G3) > SNAP_TOL else 0.0)
    if abs(b1) > SNAP_TOL: B3 = sgn_unit(B3)
    else:                  B3, H3 = 0.0, (math.copysign(1.0, H3) if abs(H3) > SNAP_TOL else 0.0)
    if abs(c1) > SNAP_TOL: C3 = sgn_unit(C3)
    else:                  C3, I3 = 0.0, (math.copysign(1.0, I3) if abs(I3) > SNAP_TOL else 0.0)

    if abs(D3) < 1e-10: D3 = 0.0
    if abs(E3) < 1e-10: E3 = 0.0
    if abs(F3) < 1e-10: F3 = 0.0
    if abs(J3) <= J_FORCE_THR: J3 = 0.0
    else:                      J3 = -1.0
    ...
    if zinv_keepI:
        # keep I as-is if we declared z-invariant during the initial rotation decision
        R = np.eye(3)

    # centre in final (rotated+scaled) canonical coords
...
def transform_point(p: np.ndarray, centre: np.ndarray, R: np.ndarray,
    ...
    """
    Centre is in CANONICAL coordinates.
    Transform: p' = (R^T @ p) / scale_factors1 - centre
    (This matches how 'centre' is printed/used in your manual checks.)
    """
