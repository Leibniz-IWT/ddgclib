...
def analyse_quadric(*coeffs: float,
    ...
    # If final linear terms are zero, the canonical center is the origin.
    if all(abs(v) <= SNAP_TOL for v in (G3, H3, I3)):
        centre_canon = np.zeros(3, float)
    else:
    ...
    for idx, row in df.iterrows():
        ...
        try:
            ...
            if is_flat_quadric(coeffs):
                ...
                B_xyz = (float(row["Bx"]), float(row["By"]), float(row["Bz"]))  # <-- fixed (single 3-tuple)
                C_xyz = (float(row["Cx"]), float(row["Cy"]), float(row["Cz"]))
                out_rows.append({
                    "triangle_id": float(tri_id),
                    # IDs + original source coords (requested)
                    "A_id": float(row["A_id"]),
