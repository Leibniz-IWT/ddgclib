...
def process_coeffs_transformed_csv(input_csv: str):
    ...
    for _, row in df.iterrows():
        ...
        # ====== CHANGE REQUESTED: use raw A,B,C directly ======
        a = A.copy()
        b = B.copy()
        c = C.copy()
        ...
        # orientation from unit directions
        side = _side_from_orientation_unitdirs(au, bu, cu)

        # ====== CHANGE REQUESTED: feed raw A,B,C to splitter ======
        A_can = A
        B_can = B
        C_can = C
        ...
        #     Vcorrection, A_o, B_o, C_o, coeffs
        # )

        # -------------------- Curved/flat area + per-vertex area split --------------------
        # High-accuracy O(1) curved area via Dunavant-7 using unit directions on the sphere
        ...
        A_flat = 0.5 * float(np.linalg.norm(np.cross(B_o - A_o, C_o - A_o)))#*scaling_factor
