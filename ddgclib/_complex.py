...
class Complex:
    ...
    def refine_star(self, v, exclude=set()):
        """
        ...
        if 1:
            ...
            for v1, v1nn in zip(vnn, v1nn):  #TODO: What the fuck " v1nn in zip(vnn, v1nn)"  ????
                vnnu = v1nn.intersection(vnn)
                if (v1 in exclude):
                    continue
                #if not ((v in exclude) or (v1 in exclude)):
               #
               # if not (v1 in exclude):
                d_v0v1 = self.split_edge(v.x, v1.x)
                d_v0v1_set.add(d_v0v1)

                for v2 in vnnu:
                    #if not ((v2 in exclude) or (v1 in exclude)):
                    d_v1v2 = self.split_edge(v1.x, v2.x)
                    d_v0v1.connect(d_v1v2)
                    d_v1v2_set.add(d_v1v2)  # Only used to track new vertices
