...
def spherical_cap_init(RadFoot, theta_p, NFoot=6, maxEdge=-1):
#Create a complex in the shape of a sphere with contact angle theta_p
  ...
  if maxEdge>0:
    for i in range(20):
      ...
      HC.V.merge_all(cdist=RadFoot*1e-3)
...
HC,bV = cone_init(vol, 1, maxEdge=.2)
measVol = sector_volume(HC)
print('sector_volume error ', (measVol-vol)/vol)
measVol = prism_volume(HC)
print('prism_volume error ', (measVol-vol)/vol)
measVol, area, centroid_height = triangle_prism_volume(HC)
print('triangle_prism_volume error ', (measVol-vol)/vol)

print('')
theta_p = np.pi/2
RadFoot = 1
RadCap = RadFoot/np.sin(theta_p)
vol = np.pi/3 * RadCap**3 * ( 2 + np.cos(theta_p) ) * ( 1 - np.cos(theta_p) )**2
print('make spherical cap complex')
HC,bV = spherical_cap_init(1, np.pi/2, maxEdge=.2)
measVol = sector_volume(HC)
print('sector_volume error ', (measVol-vol)/vol)
measVol = prism_volume(HC)
print('prism_volume error ', (measVol-vol)/vol)
